...
def extract_papers_from_daily_file():
    ...
    for section in sections:
        ...
        # Extract summary - it comes after the section title
        summary_match = re.search(r'一句话概括[:：]\s*(.*)', section)
        summary = summary_match.group(1).strip() if summary_match else "No summary available"
        
        # Extract paper info
        info_match = re.search(r'论文信息(.*?)(?=摘要|研究背景|$)', section, re.DOTALL)
        info_content = info_match.group(1) if info_match else ""
        
        # Extract arXiv ID
        arxiv_match = re.search(r'arXiv ID:\s*\[([^\]]+)\]', info_content)
        arxiv_id = arxiv_match.group(1) if arxiv_match else "Unknown"
        
        # Extract authors
        authors_match = re.search(r'作者[:：]\s*(.*)', info_content)
        authors = authors_match.group(1).strip() if authors_match else "Unknown"
        
        # Extract institutions
        institutions_match = re.search(r'机构[:：]\s*(.*)', info_content)
        institutions = institutions_match.group(1).strip() if institutions_match else "Unknown"
        
        # Extract publication date
        date_match = re.search(r'发表日期[:：]\s*(\d{4}-\d{2}-\d{2})', info_content)
        pub_date = date_match.group(1) if date_match else "Unknown"
        
        # Extract architectural analysis
        arch_match = re.search(r'架构分析(.*?)(?=实验结果|创新点|$)', section, re.DOTALL)
        arch_analysis = arch_match.group(1).strip() if arch_match else "No architectural analysis available"
        
        # Extract innovations
        innovations_match = re.search(r'创新点(.*?)(?=对具身智能算法工程师的启发|$)', section, re.DOTALL)
        innovations = innovations_match.group(1).strip() if innovations_match else "No innovations available"
        
        # Extract research background from the abstract part
        abstract_match = re.search(r'摘要(.*?)(?=架构分析|实验结果)', section, re.DOTALL)
        research_bg = abstract_match.group(1).strip() if abstract_match else "No research background available"
...
def main():
    ...
    write_to_feishu(markdown_content, today_str)
...
def write_to_feishu(content, date_str):
    """Write content to Feishu Wiki"""
    # This would typically use the feishu-doc skill
    # For now, we'll just confirm the action
    print(f"📝 Content prepared for Feishu Wiki (Document: Daily Digest {date_str})")
