...
def generate_improved_digest(papers, date_str):
    """Generate improved markdown digest with proper data"""
...
生成时间: {re.sub(r'\.\d+', '', str(__import__('datetime').datetime.now()))}
...
---
*本摘要由AI助手自动生成于{re.sub(r'\.\d+', '', str(__import__('datetime').datetime.now()))}*
"""
    
    return markdown_content

def main():
    print("🔄 Starting improved digest generation...")
    
    # Extract papers from daily files
    papers = fix_paper_data()
    ...
    markdown_content = generate_improved_digest(papers, today_str)
    ...
    print(f"📄 Improved digest saved to: {output_path}")
    
    # Update papers.ts for web app
    update_papers_ts(papers)
    
    print("✅ Improved digest generation completed!")
