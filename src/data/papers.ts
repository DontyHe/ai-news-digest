// 论文数据 - 由 cron 任务每日更新
// 更新路径: /home/Hebin/project/ai-news-digest/src/data/papers.json

export interface Paper {
  id: string;
  title: string;
  authors: string;
  date: string;
  institution: string;
  category: "embodied" | "vla" | "worldmodel" | "ai";
  summary: string;
  background: string;
  architecture: string;
  innovations: string;
  inspiration: string;
  pdfUrl: string;
  htmlUrl: string;
  figures: string[];
}

export const papers: Paper[] = [
  {
    id: "compact-2026-03-07",
    title: "Planning in 8 Tokens: A Compact Discrete Tokenizer for Latent World Model",
    authors: "Dongwon Kim, Gawon Seo, Jinsung Lee, Minsu Cho, Suha Kwak",
    institution: "KAIST",
    date: "2026-03-05",
    category: "worldmodel",
    summary: "提出CompACT离散tokenizer，将观察压缩到仅8个token，实现数量级加速规划，同时保持竞争性能，为世界模型实时部署提供实用步骤。",
    background: "世界模型为模拟环境动态提供了强大框架，但传统tokenizer将每个观察编码成数百个token，使规划既慢又资源密集。",
    architecture: "CompACT紧凑离散表示：仅8个token表示整个观察，动作条件世界模型基于压缩表示进行高效规划。",
    innovations: "• 极简token表示（仅8个token）vs 传统数百个token\n• 数量级加速规划，同时保持竞争性能\n• 为世界模型的实时部署提供实用步骤",
    inspiration: "世界模型的token压缩是实时控制的关键，与你的世界模型研究方向高度相关。",
    pdfUrl: "https://arxiv.org/pdf/2603.05438",
    htmlUrl: "https://arxiv.org/abs/2603.05438",
    figures: []
  },
  {
    id: "physiflow-2026-03-07",
    title: "PhysiFlow: Physics-Aware Humanoid Whole-Body VLA via Multi-Brain Latent Flow Matching and Robust Tracking",
    authors: "Weikai Qin, Sichen Wu, Ci Chen, Mengfan Liu, Linxi Feng",
    institution: "Carnegie Mellon University",
    date: "2026-03-05",
    category: "embodied",
    summary: "CMU提出首个将VLA与全身控制统一的人形机器人框架，通过多脑架构和潜在流匹配实现物理感知的全身协调。",
    background: "在人形机器人控制领域，将VLA与全身控制融合是重要挑战。现有方法在VLA推理效率低或缺乏有效的全身语义引导方面存在挑战。",
    architecture: "PhysiFlow多脑架构：融合VLA推理与全身控制，潜在流匹配学习机器人动力学，语义-运动意图引导，鲁棒跟踪确保稳定执行。",
    innovations: "• 首个将VLA与全身控制统一的人形机器人框架\n• 物理感知设计，兼顾语义推理与动态稳定性\n• 在动态肢体协调任务上展示可靠性",
    inspiration: "VLA+全身控制是前沿方向，多脑架构设计值得借鉴。",
    pdfUrl: "https://arxiv.org/pdf/2603.05410",
    htmlUrl: "https://arxiv.org/abs/2603.05410",
    figures: []
  },
  {
    id: "critic-loop-2026-03-07",
    title: "Critic in the Loop: A Tri-System VLA Framework for Robust Long-Horizon Manipulation",
    authors: "Pengfei Yi, Yingjie Ma, Wenjiang Xu, Yanan Hao, Shuai Gan",
    institution: "上海交通大学",
    date: "2026-03-05",
    category: "vla",
    summary: "上海交大提出Critic in the Loop框架，通过VLM大脑+VLA小脑+视觉Critic的三系统协作，在长程操作达到SOTA。",
    background: "平衡高层语义推理与低层反应控制是核心挑战。VLM在认知规划方面出色但推理延迟阻碍实时执行，VLA快速但缺乏复杂长程任务所需的语义深度。",
    architecture: "Critic in the Loop三系统架构：VLM大脑全局推理，VLA小脑反应执行，轻量级视觉Critic持续监控工作空间，动态路由根据任务状态切换控制权。",
    innovations: "• 三系统仿生架构\n• Critic驱动的自适应调度\n• 最小化昂贵VLM查询，同时增强OOD鲁棒性\n• 在长程操作基准上达到SOTA",
    inspiration: "分层VLA架构适合复杂长程任务，VLM+VLA协作是有效范式。",
    pdfUrl: "https://arxiv.org/pdf/2603.05185",
    htmlUrl: "https://arxiv.org/abs/2603.05185",
    figures: []
  },
  {
    id: "vla-interpret-2026-03-07",
    title: "Observing and Controlling Features in Vision-Language-Action Models",
    authors: "Hugo Buurmeijer, Carmen Amo Alonso, Aiden Swann, Marco Pavone",
    institution: "Stanford University",
    date: "2026-03-05",
    category: "vla",
    summary: "Stanford提出VLA特征可解释性研究，引入特征可观测性和可控性概念，为VLA调试和优化提供新工具。",
    background: "VLA架构复杂性（多模态输入/输出、transformer和diffusion头的混合性质）使得机械可解释性研究困难，LLM的洞察不能直接转移到VLA。",
    architecture: "引入两个核心概念：特征可观测性研究在表示空间中线性编码的特征，特征可控性分析如何通过输入控制这些特征。",
    innovations: "• 首个针对VLA特征可解释性的系统研究\n• 揭示VLA内部表示与输出行为的关系\n• 为VLA调试和优化提供新工具",
    inspiration: "VLA可解释性是新兴方向，对模型调试和信任建立很重要。",
    pdfUrl: "https://arxiv.org/pdf/2603.05487",
    htmlUrl: "https://arxiv.org/abs/2603.05487",
    figures: []
  },
  {
    id: "robopocket-2026-03-07",
    title: "RoboPocket: Improve Robot Policies Instantly with Your Phone",
    authors: "Junjie Fang, Wendi Chen, Han Xue, Fangyuan Zhou, Tian Le",
    institution: "Various",
    date: "2026-03-05",
    category: "embodied",
    summary: "提出使用消费级智能手机实现机器人无关的即时策略迭代，通过远程推理框架可视化策略预测来指导数据收集。",
    background: "规模化模仿学习受数据收集效率约束。手持接口虽成为可扩展数据采集方案，但主要在开环模式运行，操作者盲目收集演示不知道策略弱点。",
    architecture: "RoboPocket核心创新：远程推理框架可视化策略预测，机器人无关的即时策略迭代，单消费级智能手机实现。",
    innovations: "• 消除物理机器人执行的依赖\n• 实时反馈指导数据收集\n• 大幅提升数据收集效率",
    inspiration: "轻量化数据收集方案对规模化机器人学习很重要。",
    pdfUrl: "https://arxiv.org/pdf/2603.05504",
    htmlUrl: "https://arxiv.org/abs/2603.05504",
    figures: []
  },
  {
    id: "curobo-2026-03-07",
    title: "cuRoboV2: Dynamics-Aware Motion Generation with Depth-Fused Distance Fields for High-DoF Robots",
    authors: "Balakumar Sundaralingam, Adithyavairavan Murali, Stan Birchfield",
    institution: "NVIDIA",
    date: "2026-03-05",
    category: "embodied",
    summary: "NVIDIA发布cuRoboV2统一运动生成框架，B样条轨迹优化+GPU原生ESDF实现10倍加速和99%碰撞召回率。",
    background: "有效机器人自治需要安全、可执行、反应性的运动生成。当前方法碎片化：快速规划器输出物理不可行轨迹，反应控制器在高保真感知上挣扎。",
    architecture: "cuRoboV2三大创新：1) B样条轨迹优化强制平滑和扭矩限制；2) GPU原生TSDF/ESDF感知管道10倍更快；3) 可扩展GPU原生全身生成器。",
    innovations: "• 10倍更快，8倍更少内存（相比SOTA）\n• 达99%碰撞召回率\n• 动力学感知的运动生成",
    inspiration: "GPU加速对机器人规划至关重要，距离场方法在碰撞避免中有效。",
    pdfUrl: "https://arxiv.org/pdf/2603.05493",
    htmlUrl: "https://arxiv.org/abs/2603.05493",
    figures: []
  },
  {
    id: "ultradexgrasp-2026-03-07",
    title: "UltraDexGrasp: Learning Universal Dexterous Grasping for Bimanual Robots with Synthetic Data",
    authors: "Sizhe Yang, Yiman Xie, Zhixuan Liang, Yang Tian, Jia Zeng",
    institution: "Zhejiang University",
    date: "2026-03-05",
    category: "embodied",
    summary: "浙江大学提出UltraDexGrasp框架，使用合成数据学习通用双手灵巧抓取，解决数据瓶颈问题。",
    background: "当前机器人抓取在多策略设置和双手灵巧操作方面受限。数据是主要瓶颈，实现物理可信且几何符合的抓取是挑战。",
    architecture: "UltraDexGrasp：合成数据生成物理可信且几何符合的抓取，双手灵巧抓取策略学习，抗外部扭矩的鲁棒抓取。",
    innovations: "• 首个大规模双手灵巧抓取数据集\n• 合成数据驱动的通用抓取策略\n• 在多种物体上展示泛化能力",
    inspiration: "合成数据是解决数据瓶颈的有效途径，双手协调是灵巧操作关键。",
    pdfUrl: "https://arxiv.org/pdf/2603.05312",
    htmlUrl: "https://arxiv.org/abs/2603.05312",
    figures: []
  },
  {
    id: "omni-manip-2026-03-07",
    title: "Omni-Manip: Beyond-FOV Large-Workspace Humanoid Manipulation with Omnidirectional 3D Perception",
    authors: "Pei Qu, Zheng Li, Yufei Jia, Ziyun Liu, Liang Zhu",
    institution: "清华大学",
    date: "2026-03-05",
    category: "embodied",
    summary: "清华大学提出Omni-Manip框架，通过全向3D感知扩展人形机器人操作工作空间，解决自遮挡问题。",
    background: "人形机器人在非结构化环境中操作受感知限制，传统RGB-D解决方案存在视野狭窄和自遮挡问题。",
    architecture: "Omni-Manip：全向3D感知消除FOV限制，超越FOV的大工作空间操作，Omnidirectional 3D重建。",
    innovations: "• 首个全向3D感知人形操作框架\n• 解决自遮挡问题\n• 无需频繁基座移动",
    inspiration: "3D感知对人形机器人操作至关重要，全向视觉扩展工作空间。",
    pdfUrl: "https://arxiv.org/pdf/2603.05355",
    htmlUrl: "https://arxiv.org/abs/2603.05355",
    figures: []
  },
  {
    id: "lps-2026-03-07",
    title: "Latent Policy Steering through One-Step Flow Policies",
    authors: "Hokyun Im, Andrey Kolobov, Jianlong Fu, Youngwoon Lee",
    institution: "University of Southern California",
    date: "2026-03-05",
    category: "worldmodel",
    summary: "USC提出Latent Policy Steering方法，通过可微一步Flow策略保留原始动作空间Q梯度，解决潜在空间蒸馏的信息丢失问题。",
    background: "离线RL性能取决于回报最大化与行为约束的权衡，潜在steering提供结构化方式但现有方法存在信息丢失问题。",
    architecture: "Latent Policy Steering (LPS)：高保真潜在策略改进，通过可微一步Flow策略反向传播原始动作空间Q梯度。",
    innovations: "• 首个保留原始动作空间Q梯度的方法\n• 解决潜在空间蒸馏的信息丢失问题\n• 更稳定的收敛",
    inspiration: "离线RL的策略改进有新思路，与世界模型结合有潜力。",
    pdfUrl: "https://arxiv.org/pdf/2603.05296",
    htmlUrl: "https://arxiv.org/abs/2603.05296",
    figures: []
  },
  {
    id: "openfrontier-2026-03-07",
    title: "OpenFrontier: General Navigation with Visual-Language Grounded Frontiers",
    authors: "Esteban Padilla, Boyang Sun, Marc Pollefeys, Hermann Blum",
    institution: "ETH Zurich",
    date: "2026-03-05",
    category: "embodied",
    summary: "ETH Zurich提出OpenFrontier，将导航定义为稀疏子目标识别问题，通过视觉语言grounded frontiers实现高效目标条件导航。",
    background: "开放世界导航需要机器人适应复杂环境和灵活任务要求，传统方法依赖密集3D重建和手工目标度量限制泛化。",
    architecture: "OpenFrontier：稀疏子目标识别和可达，视觉语言grounded frontiers，高效语义先验与视觉锚定。",
    innovations: "• 稀疏表示代替密集重建\n• 高效的语义导航\n• 零样本泛化能力强",
    inspiration: "VLA在导航中的应用值得关注，语义推理与空间导航结合是趋势。",
    pdfUrl: "https://arxiv.org/pdf/2603.05377",
    htmlUrl: "https://arxiv.org/abs/2603.05377",
    figures: []
  },
  {
    id: "cowvla-2026-03-05",
    title: "Chain of World: World Model Thinking in Latent Motion",
    authors: "Fuxiang Yang, Donglin Di, Lulu Tang, Xuancheng Zhang, Lei Fan, Hao Li, Chen Wei, Tonghua Su, Baorui Ma",
    institution: "旷视科技 (Megvii)",
    date: "2026-03-03",
    category: "vla",
    summary: "提出CoWVLA新范式，统一世界模型时间推理与解耦潜在动作表示，解决了VLA模型忽略视觉动力学中预测性和时间因果结构的问题。",
    background: "VLA模型通常忽略视觉动力学中的预测性和时间因果结构。世界模型VLA通过预测未来帧解决但浪费容量重建冗余背景。潜在动作VLA紧凑编码帧间转换但缺乏时间连续动态建模。",
    architecture: "1) 预训练视频VAE作为潜在动作提取器，将视频段分解为结构和动作潜在变量；2) 预训练阶段：VLA从指令和初始帧学习，推断连续潜在动作链并预测段终止帧；3) 共微调阶段：将潜在动态与离散动作预测对齐，在统一自回归解码器中联合建模稀疏关键帧和动作序列。",
    innovations: "• 首次统一世界模型时间推理与潜在动作表示\n• 保留世界模型时间推理优势同时保持潜在动作紧凑性\n• 在机器人仿真基准优于现有世界模型和潜在动作方法",
    inspiration: "CoWVLA为VLA预训练提供了新范式，潜在动作链的设计思路可用于世界模型构建。",
    pdfUrl: "https://arxiv.org/pdf/2603.03195",
    htmlUrl: "https://arxiv.org/abs/2603.03195",
    figures: ["/figures/2603.03195_fig.png"]
  },
  {
    id: "ace-brain-2026-03-05",
    title: "ACE-Brain-0: Spatial Intelligence as a Shared Scaffold for Universal Embodiments",
    authors: "Ziyang Gong, Zehang Luo, Anke Tang, Zhe Liu, Shi Fu, Zhi Hou, et al.",
    institution: "上海人工智能实验室, 港中文, 华为诺亚",
    date: "2026-03-03",
    category: "embodied",
    summary: "首次提出空间智能作为跨智能体通用脚手架，在SSR范式下统一空间推理、自动驾驶和具身操作，在24个基准达到SOTA。",
    background: "通用具身智能需要在异构智能体（自动驾驶、机器人、无人机）间实现鲁棒泛化。现有方法训练统一模型时常遭遇长尾数据、梯度干扰和灾难性遗忘。",
    architecture: "Scaffold-Specialize-Reconcile(SSR)范式：1) 建立共享空间基础；2) 培养领域专用专家；3) 通过无数据模型合并进行调和。采用Group Relative Policy Optimization(GRPO)增强模型综合能力。",
    innovations: "• 首次提出空间智能作为跨智能体通用脚手架\n• SSR范式实现无数据模型合并，解决灾难性遗忘\n• 在24个空间和具身相关基准测试中达到SOTA",
    inspiration: "空间智能作为跨智能体迁移基础的想法值得借鉴，SSR范式可用于VLA模型的多智能体适应。",
    pdfUrl: "https://arxiv.org/pdf/2603.03198",
    htmlUrl: "https://arxiv.org/abs/2603.03198",
    figures: []
  },
  {
    id: "jala-2026-03-05",
    title: "Joint-Aligned Latent Action: Towards Scalable VLA Pretraining in the Wild",
    authors: "Hao Luo, Ye Wang, Wanpeng Zhang, Haoqi Yuan, Yicheng Feng, Haiweng Xu, Sipeng Zheng, Zongqing Lu",
    institution: "北大, 字节跳动",
    date: "2026-02-25",
    category: "vla",
    summary: "从人类操作视频学习潜在动作，绕过精确手部追踪标注，构建750万视频数据集显著提升机器人操作性能。",
    background: "VLA模型受限于大规模多样化机器人数据的稀缺性。人类操作视频是丰富替代来源，但现有方法需在精确标注的小数据集和不可靠标注的野生视频之间权衡。",
    architecture: "JALA(Jointly-Aligned Latent Actions)预训练框架：学习与逆动力学和真实动作对齐的预测动作嵌入，产生过渡感知、行为中心的潜在空间。规模化：UniHand-Mix，750万视频(>2000小时)，混合实验室和野外 footage。",
    innovations: "• 首次提出联合对齐的潜在动作概念\n• 绕过精确手部追踪标注需求\n• 在仿真和真实世界任务中显著提升机器人操作性能",
    inspiration: "JALA为从互联网视频数据预训练VLA提供了可行路径，联合对齐的潜在动作设计具有重要参考价值。",
    pdfUrl: "https://arxiv.org/pdf/2602.21736",
    htmlUrl: "https://arxiv.org/abs/2602.21736",
    figures: []
  },
  {
    id: "pose-vla-2026-03-05",
    title: "Universal Pose Pretraining for Generalizable Vision-Language-Action Policies",
    authors: "Haitao Lin, Hanyang Yu, Jingshun Zhang, He Zhang, Yonggen Ling, Ping Tan, Xiangyang Xue, Yanwei Fu",
    institution: "复旦, 上海科技大学, 港中文(深圳)",
    date: "2026-02-23",
    category: "vla",
    summary: "提出离散姿态tokens作为通用表示，两阶段预训练解耦空间 grounding 与动作对齐，在RoboTwin达79.5%成功率。",
    background: "现有VLA模型常遭受特征崩溃和低训练效率，因为将高级感知与稀疏、智能体特定的动作监督纠缠。依赖为VQA优化的VLM backbone，忽略决定不同动作模式的微妙3D状态变化。",
    architecture: "Pose-VLA解耦范式：1) 预训练阶段：在统一相机中心空间提取通用3D空间先验；2) 后训练阶段：在智能体特定动作空间中进行高效智能体对齐。引入离散姿态tokens作为通用表示。",
    innovations: "• 首次提出通用姿态预训练范式\n• 离散姿态tokens作为跨智能体通用表示\n• 在RoboTwin 2.0达79.5%平均成功率，LIBERO达96.0%",
    inspiration: "姿态预训练思路可用于VLA的空间理解能力提升，离散tokens设计具有通用性。",
    pdfUrl: "https://arxiv.org/pdf/2602.19710",
    htmlUrl: "https://arxiv.org/abs/2602.19710",
    figures: []
  },
  {
    id: "tether-2026-03-05",
    title: "Tether: Autonomous Functional Play with Correspondence-Driven Trajectory Warping",
    authors: "William Liang, Sam Wang, Hung-Ju Wang, Osbert Bastani, Yecheng Jason Ma, Dinesh Jayaraman",
    institution: "UPenn, NVIDIA Research",
    date: "2026-03-03",
    category: "embodied",
    summary: "首个实现真实世界数小时自主多任务play的方法，基于语义关键点动作扭曲，从10个演示生成1000+轨迹。",
    background: "从交互和经验中学习是机器人学核心挑战，提供劳动密集型人类演示的可扩展替代方案。实现play需要：对多样化分布外环境状态鲁棒的政策；持续产生有用机器人经验的程序。",
    architecture: "Tether两部分：1) 开放循环策略：将少量源演示(≤10个)的动作通过语义关键点对应 warp 到目标场景；2) 自主功能play：VLM引导的持续循环：任务选择→执行→评估→改进。",
    innovations: "• 首个在真实世界中实现数小时自主多任务play的方法\n• 基于语义关键点的动作扭曲策略，数据效率极高\n• 最终产生1000+专家级轨迹，性能媲美人类收集的演示",
    inspiration: "自主数据收集策略可用于扩展机器人数据集，关键点对应方法为少样本泛化提供思路。",
    pdfUrl: "https://arxiv.org/pdf/2603.03278",
    htmlUrl: "https://arxiv.org/abs/2603.03278",
    figures: ["/figures/2603.03278_fig.png"]
  },
  {
    id: "utonia-2026-03-05",
    title: "Utonia: Toward One Encoder for All Point Clouds",
    authors: "Yujia Zhang, Xiaoyang Wu, Yunhan Yang, Xianzhe Fan, Han Li, Yuechen Zhang, Zehao Huang, Naiyan Wang, Hengshuang Zhao",
    institution: "港中文, 上海AI Lab, 商汤",
    date: "2026-03-03",
    category: "worldmodel",
    summary: "首个跨领域点云基础模型，联合训练产生emergent具身推理能力，为VLA提供统一3D感知编码器。",
    background: "来自不同领域的点云可以汇聚成使它们都受益的单一模型。现有点云模型通常针对单一领域设计，缺乏跨领域统一。",
    architecture: "Utonia：首个跨领域训练的自监督点Transformer编码器。领域涵盖：遥感、户外LiDAR、室内RGB-D序列、物体中心CAD模型、从纯RGB视频提升的点云。联合训练产生有趣 emergent behaviors。",
    innovations: "• 首个跨领域点云foundation model\n• 联合训练产生 emergent 具身和多模态推理能力\n• 为稀疏3D数据foundation model奠定基础",
    inspiration: "跨领域联合训练策略可用于提升VLA的3D感知能力，Utonia特征可直接用于VLA backbone。",
    pdfUrl: "https://arxiv.org/pdf/2603.03283",
    htmlUrl: "https://arxiv.org/abs/2603.03283",
    figures: ["/figures/2603.03283_fig.png"]
  },
  {
    id: "cmoe-2026-03-05",
    title: "CMoE: Contrastive Mixture of Experts for Motion Control and Terrain Adaptation of Humanoid Robots",
    authors: "Shihao Ma, Hongjin Chen, Zijun Xu, Yi Zhao, Ke Wu, Ruichen Yang, Leyao Zou, Zhongxue Ding, Wenchao Ding",
    institution: "上海交通大学, 清华大学",
    date: "2026-03-03",
    category: "embodied",
    summary: "对比学习约束解决MoE专家激活均匀化问题，机器人可穿越20cm台阶和80cm间隙。",
    background: "人形机器人需要在具有急剧过渡的多样复杂地形中自主导航。Vanilla MoE框架理论上能建模多样化地形特征，但实际中门控网络在不同地形上呈现几乎均匀的专家激活。",
    architecture: "CMoE：新型单阶段强化学习框架，集成对比学习细化专家激活分布。通过对比约束，最大化同一地形内专家激活的一致性，最小化不同地形间的相似性。",
    innovations: "• 首次将对比学习引入MoE的门控网络\n• 有效解决专家激活均匀化问题\n• 机器人可穿越高达20cm的连续台阶和80cm宽的间隙",
    inspiration: "对比学习约束可应用于VLA的多任务学习，增强任务特定专家的专业化。",
    pdfUrl: "https://arxiv.org/pdf/2603.03067",
    htmlUrl: "https://arxiv.org/abs/2603.03067",
    figures: []
  },
  {
    id: "ma-conav-2026-03-05",
    title: "MA-CoNav: A Master-Slave Multi-Agent Framework with Hierarchical Collaboration and Dual-Level Reflection for Long-Horizon Embodied VLN",
    authors: "Ling Luo, Qianqian Bai",
    institution: "哈尔滨工业大学",
    date: "2026-03-03",
    category: "embodied",
    summary: "主从层级多智能体协作框架，全方面超越现有VLN方法，在真实机器人无微调实现长程导航。",
    background: "视觉语言导航(VLN)需要在陌生环境中基于复杂语言指令执行长程导航。现有方法常因单一智能体的认知过载而在复杂长距离任务中遭受感知失真和决策漂移。",
    architecture: "MA-CoNav：多智能体协作框架。'主从'层级架构：Master Agent全局编排，Observation Agent环境描述，Planning Agent任务分解和动态验证，Execution Agent同时建图和动作，Memory Agent结构化经验管理。'局部-全局'双阶段反射机制。",
    innovations: "• 分布式认知理论启发的多智能体架构\n• 首次在真实机器人(Limo Pro)上无场景特定微调实现长程VLN\n• 全面超越现有主流VLN方法",
    inspiration: "主从协作架构可用于复杂环境中的VLA任务分解，反射机制增强长程任务可靠性。",
    pdfUrl: "https://arxiv.org/pdf/2603.03024",
    htmlUrl: "https://arxiv.org/abs/2603.03024",
    figures: []
  },
  {
    id: "adm-dp-2026-03-05",
    title: "ADM-DP: Adaptive Dynamic Modality Diffusion Policy through Vision-Tactile-Graph Fusion for Multi-Agent Manipulation",
    authors: "Enyi Wang, Wen Fan, Dandan Zhang",
    institution: "清华大学",
    date: "2026-02-25",
    category: "embodied",
    summary: "首个集成视觉-触觉-图模态的多智能体扩散策略，7个任务达12-25%性能提升。",
    background: "多智能体机器人在协调、抓取稳定性、避免共享工作空间碰撞方面面临挑战。",
    architecture: "ADM-DP：1) 增强视觉编码器通过FiLM调制融合RGB和点云特征；2) 触觉引导抓取策略使用FSR反馈检测接触不足并触发修正；3) 基于图的碰撞编码器利用多智能体共享TCP位置；4) 自适应模态注意力机制动态重加权模态。",
    innovations: "• 首个集成视觉、触觉、图模态的多智能体扩散策略\n• FiLM调制增强视觉感知，触觉反馈实时修正抓取\n• 在7个多智能体任务中达到12-25%性能提升",
    inspiration: "多模态融合策略可用于增强VLA的感知能力，自适应注意力机制值得借鉴。",
    pdfUrl: "https://arxiv.org/pdf/2602.21622",
    htmlUrl: "https://arxiv.org/abs/2602.21622",
    figures: []
  },
  {
    id: "denoising-pf-2026-03-05",
    title: "Denoising Particle Filters: Learning State Estimation with Single-Step Objectives",
    authors: "Lennart Röstel, Berthold Bäuml",
    institution: "TUM (慕尼黑工业大学)",
    date: "2026-02-23",
    category: "worldmodel",
    summary: "将去噪分数匹配应用于粒子滤波，单步训练目标提高效率，保留经典滤波可组合性。",
    background: "基于学习的机器人状态估计常被视为序列建模问题。模型难以解释且训练昂贵，因为训练需要在时间上展开预测序列。",
    architecture: "新型粒子滤波算法：模型从单个状态转换中训练，充分利用马尔可夫性质。隐式学习测量模型，最小化去噪分数匹配目标。推理时，学习到的去噪器与动态模型一起近似求解每一步的贝叶斯滤波方程。",
    innovations: "• 首个将去噪分数匹配应用于粒子滤波的方法\n• 保留经典滤波算法的可组合性，可纳入先验信息\n• 在仿真机器人状态估计任务中达到与端到端基线相当的性能",
    inspiration: "去噪分数匹配可用于学习VLA世界模型的动态模型，单步训练目标提高训练效率。",
    pdfUrl: "https://arxiv.org/pdf/2602.19651",
    htmlUrl: "https://arxiv.org/abs/2602.19651",
    figures: []
  },
  {
    id: "utonia-2026-03-04",
    title: "Utonia: Toward One Encoder for All Point Clouds",
    authors: "Yujia Zhang, Xiaoyang Wu, Yunhan Yang, Xianzhe Fan, Han Li, Yuechen Zhang, Zehao Huang, Naiyan Wang, Hengshuang Zhao",
    institution: "Shanghai AI Lab, 港中文, 浙大",
    date: "2026-03-03",
    category: "worldmodel",
    summary: "提出Utonia，一个统一的点云自监督预训练编码器，能够跨域（遥感、户外LiDAR、室内RGB-D、CAD模型、视频点云）学习一致的特征表示，并可迁移用于具身智能和VLA任务。",
    background: "当前点云处理通常针对特定领域训练独立模型，缺乏跨域统一能力。不同域的点云在感知几何、密度、先验知识上差异巨大。",
    architecture: "统一的Point Transformer编码器架构 + 跨域自监督预训练策略。冻结特征可直接用于下游任务。",
    innovations: "• 首个跨所有点云域的统一自监督表示学习框架\n• 发现联合训练产生\"涌现行为\"\n• 可作为稀疏3D数据的Foundation Model",
    inspiration: "Utonia的特征可直接用于VLA模型的3D感知输入，支持embodied和multimodal reasoning，集成到VLA策略中可提升机器人操作能力。",
    pdfUrl: "https://arxiv.org/pdf/2603.03283",
    htmlUrl: "https://arxiv.org/abs/2603.03283",
    figures: ["/figures/2603.03283_fig.png"]
  },
  {
    id: "miburi-2026-03-04",
    title: "MIBURI: Towards Expressive Interactive Gesture Synthesis",
    authors: "M. Hamza Mughal, Rishabh Dabral, Vera Demberg, Christian Theobalt",
    institution: "MPI Informatics, TU Darmstadt",
    date: "2026-03-03",
    category: "embodied",
    summary: "提出MIBURI，首个在线因果框架，实时生成与语音同步的富有表现力的全身手势和面部表情，用于具身对话智能体。",
    background: "现有LLM-based对话智能体缺乏具身性和表达性手势，而传统共手势合成方法依赖未来语音上下文、推理时间长，不适合实时交互。",
    architecture: "身体部位感知的Gesture Codecs，将层级运动细节编码为多级离散token + 二维因果框架自回归生成 + 基于LLM speech-text embeddings的条件生成。",
    innovations: "• 首个在线、因果的实时全身手势生成框架\n• 建模时序动态和部位级运动层级\n• 辅助目标防止收敛到静态姿势",
    inspiration: "该工作展示了对具身智能体\"表达性\"的重要性，手势生成可作为ECAs的组件，提升人机交互自然度。",
    pdfUrl: "https://arxiv.org/pdf/2603.03282",
    htmlUrl: "https://arxiv.org/abs/2603.03282",
    figures: ["/figures/2603.03282_fig.png"]
  },
  {
    id: "tether-2026-03-04",
    title: "Tether: Autonomous Functional Play with Correspondence-Driven Trajectory Warping",
    authors: "William Liang, Sam Wang, Hung-Ju Wang, Osbert Bastani, Yecheng Jason Ma, Dinesh Jayaraman",
    institution: "UPenn, NVIDIA, Carnegie Mellon",
    date: "2026-03-03",
    category: "embodied",
    summary: "提出Tether，通过语义关键点Correspondence-Driven的轨迹warping实现数据高效、功能导向的自主玩耍学习，仅需≤10个源演示即可泛化到新场景。",
    background: "机器人通过\"玩耍\"自主学习是规模化替代人工演示的关键，但面临策略需对多样、分布外环境状态鲁棒，以及持续产生有用机器人体验的挑战。",
    architecture: "开放环路策略：基于语义关键点Correspondence warping动作 + VLM引导的自主玩耍循环：任务选择→执行→评估→改进。",
    innovations: "• 仅需极少演示（≤10个）即可泛化到显著空间和语义变化\n• 首次在真实世界实现数小时自主多任务玩耍\n• 持续提升闭环模仿策略性能，产生>1000条专家级轨迹",
    inspiration: "该工作展示了如何利用VLM+少量演示实现机器人自主数据收集，对构建大规模机器人数据集有重要参考价值。",
    pdfUrl: "https://arxiv.org/pdf/2603.03278",
    htmlUrl: "https://arxiv.org/abs/2603.03278",
    figures: ["/figures/2603.03278_fig.png"]
  },
  {
    id: "ultra-2026-03-04",
    title: "ULTRA: Unified Multimodal Control for Autonomous Humanoid Whole-Body Loco-Manipulation",
    authors: "Xialin He, Sirui Li, Xinyao Yao, Runpei Dong, Liuyu Bian, Yu-Xiong Wang, Liang-Yan Gui",
    institution: "上交, 浙大, UIUC",
    date: "2026-03-03",
    category: "embodied",
    summary: "提出ULTRA，统一框架实现类人机器人全车身locomanipulation，通过物理驱动的神经重定向+强化学习微调，支持密集参考和稀疏任务规范。",
    background: "现有类人机器人方法受限于重定向数据稀缺、难扩展到大技能库、依赖跟踪预定义运动而非从感知生成行为。",
    architecture: "物理驱动神经重定向：将大规模动捕数据转换到类人实体 + 统一多模态控制器：支持密集参考+稀疏任务规范 + RL微调扩展覆盖范围和鲁棒性。",
    innovations: "• 首个支持端到端目标条件全车身locomanipulation的框架\n• 无需测试时参考动作\n• 在Unitree G1真机上验证",
    inspiration: "该工作对VLA模型在人形机器人上的部署有借鉴意义，特别是稀疏意图到全身动作的映射设计。",
    pdfUrl: "https://arxiv.org/pdf/2603.03279",
    htmlUrl: "https://arxiv.org/abs/2603.03279",
    figures: ["/figures/2603.03279_fig.png"]
  },
  {
    id: "homni-2026-03-04",
    title: "HoMMI: Learning Whole-Body Mobile Manipulation from Human Demonstrations",
    authors: "Xiaomeng Xu, Jisang Park, Han Zhang, Eric Cousineau, Aditya Bhat, Jose Barreiros, Dian Wang, Shuran Song",
    institution: "Columbia University",
    date: "2026-03-03",
    category: "embodied",
    summary: "提出HoMMI，从无机器人的人类演示学习全身移动操作，通过UMI+自我中心感知实现便携、无机器人、可扩展的数据收集，并设计跨实体手眼策略弥补人和机器人的感知-动作差距。",
    background: "移动操作需要双手、全身协调、导航和主动感知，数据收集困难。现有方法依赖特定机器人平台，难以泛化。",
    architecture: "UMI+自我中心感知：捕获全局上下文 + 跨实体手眼策略设计：实体无关视觉表示、放大的头部动作表示、全身控制器。",
    innovations: "• 首个从人类演示学习全身移动操作的工作\n• 解决人类到机器人感知-动作空间的实体差距\n• 支持长程移动操作任务",
    inspiration: "HoMMI展示了如何利用人类演示数据训练机器人策略，其跨实体设计对VLA模型的sim2real迁移有重要参考。",
    pdfUrl: "https://arxiv.org/pdf/2603.03243",
    htmlUrl: "https://arxiv.org/abs/2603.03243",
    figures: ["/figures/2603.03243_fig.png"]
  },
  {
    id: "stemvla-2026-02-27",
    title: "StemVLA: An Open-Source Vision-Language-Action Model with Future 3D Spatial Geometry Knowledge and 4D Historical Representation",
    authors: "Jiasong Xiao, Yutao She, Kai Li, Yuyang Sha, Ziang Cheng, Ziang Tong",
    institution: "Various",
    date: "2026-02-27",
    category: "vla",
    summary: "StemVLA 显式整合未来导向的3D空间知识和历史4D时空表示，通过预测结构化的3D未来世界知识来增强动作预测。",
    background: "现有VLA模型主要依赖从2D视觉输入到动作序列的直接映射，没有显式建模底层3D空间结构或时间世界动态，可能限制空间推理和长程决策。",
    architecture: "StemVLA 包含三个核心组件：1) 预测结构化的3D未来空间-几何世界知识；2) 将历史图像帧输入预训练的video-geometry transformer主干提取隐式3D世界表示；3) 使用时间注意力模块(VideoFormer)跨时间聚合形成统一的4D历史时空表示。",
    innovations: "• 显式3D未来预测: 预测未来场景几何和物体配置\n• 4D历史表示: 联合建模2D观察、预测3D未来结构和4D时间动态\n• CALVIN基准: 在ABC-D基准上实现SOTA性能",
    inspiration: "StemVLA的3D空间建模和4D时空表示方法对你研究VLA的长程推理能力有重要参考价值，其显式未来预测可能是突破当前VLA瓶颈的关键。",
    pdfUrl: "https://arxiv.org/pdf/2602.23721",
    htmlUrl: "https://arxiv.org/abs/2602.23721",
    figures: []
  },
  {
    id: "favla-2026-02-27",
    title: "FAVLA: A Force-Adaptive Fast-Slow VLA model for Contact-Rich Robotic Manipulation",
    authors: "Yao Li, Peiyuan Tang, Wuyang Zhang, Chengyang Zhu, Yifan Duan, Weikai Shi, Xiaodong Zhang, Zijiang Yang, Jianmin Ji, Yanyong Zhang",
    institution: "Various",
    date: "2026-02-27",
    category: "vla",
    summary: "FAVLA通过快慢双环路架构解耦力反馈感知与控制，实现接触丰富操纵的自适应频率调度。",
    background: "力/力矩反馈可以显著改善接触丰富操纵的VLA模型，但现有方法在单一工作频率融合所有模态，忽略了真实机器人传感器的不匹配采样率。",
    architecture: "FAVLA采用快慢双环路：慢速VLM以固定低频运行编码模态并预测近未来力变化；快速动作专家(AE)以可变高频执行，根据最新力序列数据生成反应动作。引入力适配器将高频力特征注入多个AE层，并基于VLM预测的力变化自适应调度AE执行频率。",
    innovations: "• 快慢双环路: 力反馈感知与控制解耦\n• 自适应频率调度: 根据力变化动态调整控制频率\n• 接触丰富任务: 显著优于基线方法",
    inspiration: "FAVLA的快慢架构设计对你研究力反馈控制有重要参考价值，其自适应频率调度策略是平衡计算效率和控制精度的有效方法。",
    pdfUrl: "https://arxiv.org/pdf/2602.23648",
    htmlUrl: "https://arxiv.org/abs/2602.23648",
    figures: []
  },
  {
    id: "brittle-grounding-2026-02-27",
    title: "Robust Skills, Brittle Grounding: Diagnosing Restricted Generalization in Vision-Language Action Policies via Multi-Object Picking",
    authors: "David Emukpere, Romain Deffayet, Jean-Michel Renders",
    institution: "Various",
    date: "2026-02-27",
    category: "vla",
    summary: "研究揭示VLA策略的指令-grounded泛化存在限制，操纵技能获取与指令跟随可能解耦。",
    background: "VLA策略在少量演示数据下能取得较强的操纵基准测试成绩，但这种表现究竟源于robust的语言-物体grounding，还是依赖于物体-位置相关性仍不清楚。",
    architecture: "本文提出受控的多物体拾取研究，逐步增加物体放置变异性直至完全随机化，并评估打破熟悉关联的held-out物体-位置配对。测试了包括SmolVLA和π₀.₅在内的代表性VLA策略。",
    innovations: "• 发现执行操纵原语比指令条件下的任务成功更可靠\n• 操纵技能获取与指令跟随是解耦的\n• 建议增加任务阶梯和分解指标",
    inspiration: "VLA模型的泛化能力可能存在'虚假繁荣'，需要更严格的测试。指令-grounded泛化值得单独评估。",
    pdfUrl: "https://arxiv.org/pdf/2602.24143",
    htmlUrl: "https://arxiv.org/abs/2602.24143",
    figures: []
  },
  {
    id: "planning-observation-2026-02-27",
    title: "Planning from Observation and Interaction",
    authors: "Tyler Han, Siyang Shen, Rohan Baijal, Harine Ravichandiran, Bat Nemekhbold, Kevin Huang, Sanghun Jung, Byron Boots",
    institution: "University of Washington",
    date: "2026-02-27",
    category: "embodied",
    summary: "提出基于规划的逆强化学习算法，仅从观察和交互中进行世界建模，实现一小时内从零学习图像操纵任务。",
    background: "观察学习要求智能体仅通过观察执行任务来学习。在真实机器人学习中，无法假设可以获得手设计的奖励和演示者动作。",
    architecture: "提出基于规划的IRL算法，从观察和交互中进行世界建模。完全在真实世界中进行实验，证明该范式可以在一小时内从零学习基于图像的操纵任务，不需要任何先验知识、预训练或除任务观察之外的任何数据。",
    innovations: "• 无先验学习: 不假设任何先验知识或预训练\n• 在线迁移学习: 学习的世界模型表示能够进行在线迁移学习\n• 高样本效率: 与IRL、RL和BC相比显著提高样本效率",
    inspiration: "从观察中学习世界模型是数据受限设置下的可行路径，在线世界建模和从观察与交互中规划是实际可行的。",
    pdfUrl: "https://arxiv.org/pdf/2602.24121",
    htmlUrl: "https://arxiv.org/abs/2602.24121",
    figures: []
  },
  {
    id: "foundation-worldmodel-2026-02-27",
    title: "Foundation World Models for Agents that Learn, Verify, and Adapt Reliably Beyond Static Environments",
    authors: "Florent Delgrange",
    institution: "Various",
    date: "2026-02-27",
    category: "worldmodel",
    summary: "提出基础世界模型的愿景：持久的、组合的表示，统一强化学习、反应式/程序合成和抽象机制。",
    background: "下一代自主智能体不仅要学习高效，还要在开放世界中可靠地行动并适应行为变化。标准方法假设固定任务和环境，限制了世界模型支持必须随条件变化更新策略的智能体的能力。",
    architecture: "围绕四个组件构建议程：1) 从规范中学习奖励模型；2) 学习中集成的自适应形式验证；3) 在线抽象校准以量化模型预测的可靠性；4) 验证器引导的测试时合成和世界模型生成。",
    innovations: "• 形式验证集成: 将形式验证集成到学习过程中\n• 测试时合成: 验证器引导的测试时合成和世界模型生成\n• 可靠适应: 使智能体能够从少量交互中推导新策略",
    inspiration: "基础世界模型可能是学习、推理和适应的统一底层，可靠性验证与学习过程的结合是重要方向。",
    pdfUrl: "https://arxiv.org/pdf/2602.23997",
    htmlUrl: "https://arxiv.org/abs/2602.23997",
    figures: []
  },
  {
    id: "aoe-2026-02-27",
    title: "AoE: Always-on Egocentric Human Video Collection for Embodied AI",
    authors: "Large Team",
    institution: "Various",
    date: "2026-02-27",
    category: "embodied",
    summary: "提出AoE数据收集系统，利用人类自身和智能手机进行低门槛、大规模自我中心数据收集。",
    background: "具身基础模型需要大规模、高质量的真实世界交互数据进行预训练和扩展，但现有数据收集方法成本高、硬件依赖复杂、交互范围有限。",
    architecture: "AoE系统包含：1) 人体颈挂式智能手机支架，低门槛大规模自我中心数据收集；2) 跨平台移动应用，利用设备端计算进行实时处理；3) 云端托管自动标注和过滤管道，将原始视频转换为高质量训练数据。",
    innovations: "• 云边协同: 低成本、高效率、场景无关的真实世界交互数据收集\n• 分布式收集: 任何人、任何时间、任何地点都能进行数据收集\n• 下游任务: 高质量自我中心数据显著提升真实世界泛化能力",
    inspiration: "人类自我中心视频是具身智能数据的宝贵来源，低成本数据收集对规模化至关重要。",
    pdfUrl: "https://arxiv.org/pdf/2602.23893",
    htmlUrl: "https://arxiv.org/abs/2602.23893",
    figures: []
  },
  {
    id: "omnixtreme-2026-02-27",
    title: "OmniXtreme: Breaking the Generality Barrier in High-Dynamic Humanoid Control",
    authors: "Various",
    institution: "Various",
    date: "2026-02-27",
    category: "embodied",
    summary: "提出OmniXtreme框架，解耦通用运动技能学习与物理技能精炼，实现高动态人形机器人控制。",
    background: "高保真运动跟踪是通用化、类人运动技能的终极测试，但当前策略存在' generality barrier'：随着运动库规模增大，跟踪保真度不可避免下降。",
    architecture: "OmniXtreme采用两阶段方法：1) 使用flow-matching策略和高效架构进行通用运动技能学习，无需干扰密集的多运动RL优化；2)  actuation-aware精炼阶段确保在物理硬件上的鲁棒性能。",
    innovations: "• 技能解耦: 将运动技能学习与物理精炼分离\n• 高保真跟踪: 在多样化、高难度数据集上保持高保真跟踪\n• 真实机器人: 在真实机器人上成功执行多种极限动作",
    inspiration: "OmniXtreme解决了高动态人形控制中长期存在的保真度-可扩展性权衡问题。",
    pdfUrl: "https://arxiv.org/pdf/2602.23843",
    htmlUrl: "https://arxiv.org/abs/2602.23843",
    figures: []
  },
  {
    id: "safegen-llm-2026-02-27",
    title: "SafeGen-LLM: Enhancing Safety Generalization in Task Planning for Robotic Systems",
    authors: "Various",
    institution: "Various",
    date: "2026-02-27",
    category: "embodied",
    summary: "提出安全可泛化LLM，增强任务规划的安全满足性并泛化到新的安全属性。",
    background: "安全关键任务规划在机器人系统中仍具挑战：经典规划器可扩展性差，基于RL的方法泛化能力弱，基础LLM无法保证安全性。",
    architecture: "SafeGen-LLM包含两阶段后训练框架：1) 在合规规划数据集上进行监督微调(SFT)学习规划语法和语义；2) 使用基于形式验证的细粒度奖励机器人和课程学习进行GRPO训练，增强安全对齐和处理复杂任务的能力。",
    innovations: "• 安全泛化: 不仅增强任务计划的安全满足性，还泛化到新安全属性\n• 两阶段训练: SFT + GRPO的组合\n• 多域验证: 在多域规划任务和多种输入格式上优于基线",
    inspiration: "SafeGen-LLM在安全关键机器人任务规划中展示了有前景的泛化能力。",
    pdfUrl: "https://arxiv.org/pdf/2602.24235",
    htmlUrl: "https://arxiv.org/abs/2602.24235",
    figures: []
  },
  {
    id: "lerobot-2026-02-26",
    title: "LeRobot: An Open-Source Library for End-to-End Robot Learning",
    authors: "Remi Cadene, Simon Aliberts, Francesco Capolino, et al.",
    institution: "Hugging Face",
    date: "2026-02-26",
    category: "embodied",
    summary: "Hugging Face发布的开源机器人学习库，集成从底层电机控制到大规模数据集采集的全栈能力。",
    background: "机器人学习领域发展迅速，但工具链碎片化严重。现有工具通常是封闭源码、仅针对特定子模块设计，阻碍了端到端机器人学习的研究和应用。",
    architecture: "LeRobot是Hugging Face发布的开源库，集成从底层电机控制通信到大规模数据集采集、存储和流式的全栈能力。支持多种SOTA机器人学习算法（行为克隆、扩散策略、RL等），提供异步推理栈。",
    innovations: "• 全栈集成: 首个覆盖完整机器人学习流程的开源库\n• 硬件兼容性: 支持主流 affordable 硬件平台，同时保持可扩展性\n• 数据驱动: 强调可扩展学习方法，支持大规模数据训练",
    inspiration: "如你研究VLA模型的训练流程，LeRobot的端到端数据 pipeline 值得参考，其异步推理架构对实时控制任务有借鉴意义。",
    pdfUrl: "https://arxiv.org/pdf/2602.22818",
    htmlUrl: "https://arxiv.org/abs/2602.22818",
    figures: ["/figures/lerobot_fig1.png"]
  },
  {
    id: "geoworld-2026-02-26",
    title: "GeoWorld: Geometric World Models",
    authors: "Zeyu Zhang, Danning Li, Ian Reid, Richard Hartley",
    institution: "University of Oxford",
    date: "2026-02-26",
    category: "worldmodel",
    summary: "引入双曲JEPA将潜在表示从欧几里得空间映射到双曲流形，解决长程预测退化问题。",
    background: "基于能量的预测世界模型通过在潜在能量景观上进行多步视觉规划。然而现有方法面临两大挑战：1) 潜在表示通常在欧几里得空间学习，忽略状态间的几何和层次结构；2) 长程预测能力不足，跨长 horizon 会快速退化。",
    architecture: "GeoWorld 引入双曲JEPA (Hyperbolic JEPA)，将潜在表示从欧几里得空间映射到双曲流形，保持几何结构和层次关系。配合几何强化学习进行基于能量的优化，实现双曲潜在空间中的稳定多步规划。",
    innovations: "• 双曲流形建模: 首个将双曲几何融入世界模型的工作\n• 层次结构保持: 解决传统欧几里得空间无法建模状态层次关系的问题\n• 长程预测: 在3步/4步规划任务上比 V-JEPA 2 提升约3%/2% 成功率",
    inspiration: "你研究世界模型方向，GeoWorld 的双曲空间建模方法可能是突破长程预测瓶颈的新思路，值得关注。",
    pdfUrl: "https://arxiv.org/pdf/2602.23058",
    htmlUrl: "https://arxiv.org/abs/2602.23058",
    figures: ["/figures/geoworld_fig1.png", "/figures/geoworld_fig2.png"]
  },
  {
    id: "dysl-vla-2026-02-26",
    title: "DySL-VLA: Efficient Vision-Language-Action Model Inference via Dynamic-Static Layer-Skipping",
    authors: "Zebin Yang, Yijiahao Qi, Tong Xie, Bo Yu, Shaoshan Liu, Meng Li",
    institution: "Shanghai Jiao Tong University",
    date: "2026-02-26",
    category: "vla",
    summary: "通过动态静态层跳过机制，实现VLA模型3.75x推理加速。",
    background: "VLA模型在机器人操作任务上表现出色，但计算成本过高，难以满足实时部署需求。",
    architecture: "DySL-VLA 核心洞察：任务中不同动作的重要性不同。关键步骤需要高精度，次要步骤可容忍更大方差。基于此，将VLA层分为两类：\n• 信息层 (Informative Layers): 始终执行\n• 增量层 (Incremental Layers): 可选择性跳过\n引入先验-后验跳过引导机制决定何时启动层跳过，并提出跳过感知的两阶段知识蒸馏训练。",
    innovations: "• 动态推理: 根据动作重要性动态调整计算量\n• 推理加速: 在 iso-accuracy 下达到 3.75x 加速\n• 参数减少: 可训练参数减少 85.7x",
    inspiration: "如你关注VLA模型的部署效率，DySL-VLA 的动态层跳过机制比传统模型压缩方法更细粒度，其知识蒸馏策略也值得参考。",
    pdfUrl: "https://arxiv.org/pdf/2602.22896",
    htmlUrl: "https://arxiv.org/abs/2602.22896",
    figures: ["/figures/dysl_vla_fig1.png", "/figures/dysl_vla_fig2.png", "/figures/dysl_vla_fig3.png"]
  },
  {
    id: "sparr-2026-02-26",
    title: "SPARR: Simulation-based Policies with Asymmetric Real-world Residuals for Assembly",
    authors: "Yijie Guo, Iretiayo Akinola, Lars Johannsmeier, Hugo Hadfield, Abhishek Gupta, Yashraj Narang",
    institution: "Google DeepMind",
    date: "2026-02-26",
    category: "embodied",
    summary: "提出混合仿真+真实世界的非对称残差策略，解决机器人装配中的 sim-to-real 差距。",
    background: "机器人装配是长期挑战，需要精确的接触丰富操作。Sim-to-real方法存在性能下降，纯real-world RL依赖大量人工监督且泛化能力差。",
    architecture: "SPARR 提出混合方案：\n• Base Policy: 在仿真中训练，使用底层状态观测和密集奖励，提供强先验\n• Residual Policy: 在真实世界学习，使用视觉观测和稀疏奖励，补偿动力学差异和传感器噪声",
    innovations: "• 非对称残差: 仿真策略和真实世界残差的非对称组合\n• 零样本 vs RL: 比SOTA零样本sim-to-real提升38.4%成功率，周期时间减少29.7%\n• 无需人工 expertise: 摆脱对人工监督的依赖",
    inspiration: "SPARR 的非对称残差思路可用于你研究的VLA模型，特别是处理sim2real gap 时，基础策略+残差补偿的范式值得关注。",
    pdfUrl: "https://arxiv.org/pdf/2602.23253",
    htmlUrl: "https://arxiv.org/abs/2602.23253",
    figures: []
  },
  {
    id: "cebench-2026-02-25",
    title: "Rethinking the Practicality of Vision-Language-Action Model: A Comprehensive Benchmark and An Improved Baseline",
    authors: "Wenxuan Song, Jiayi Chen",
    institution: "Tsinghua University",
    date: "2026-02-25",
    category: "vla",
    summary: "发布首个VLA实用性 benchmark CEBench 和轻量级 VLA 模型 LLaVA-VLA。",
    background: "现有VLA模型存在三大问题：参数规模过大、预训练成本高、对不同 embodiment 适用性差。",
    architecture: "• CEBench: 新 benchmark，覆盖多种 embodiment（仿真+真实），包含14.4k仿真轨迹和1.6k真实轨迹\n• LLaVA-VLA: 轻量级 VLA，消费级GPU可部署\n  - 紧凑 VLM backbone + 多视角感知 + 本体感知 tokenization + action chunking\n  - 两阶段训练：后训练 + 微调\n  - 统一导航和操作的动作空间",
    innovations: "• 实用性 benchmark: 首个全面评估VLA实用性的基准\n• 消费级部署: 首个端到端支持移动操作的VLA模型\n• 无大Scale预训练: 摆脱对大规模预训练的依赖",
    inspiration: "CEBench 的评估框架和 LLaVA-VLA 的轻量化设计对你做 VLA 研究有直接参考价值，特别是多 embodiment 泛化性的评估方法。",
    pdfUrl: "https://arxiv.org/pdf/2602.22663",
    htmlUrl: "https://arxiv.org/abs/2602.22663",
    figures: []
  }
];

export default papers;
