...
export const papers: Paper[] = [
  {
    ...
    date: "2026-06-04",
    category: "embodied",
    summary: "World Models for Embodied AI in Complex Environments提出了一种创新的方法来解决具身智能领域的核心挑战。论文提出了统一的时空世界模型框架，同时处理短期感知和长期预测，引入了注意力机制来处理环境中的关键对象和动态变化，在多个具身智能基准测试上取得了state-of-the-art的结果。",
    background: "当前具身智能系统面临着环境理解、长期规划和动作生成的挑战。本文针对这些问题，提出了一种新的世界模型架构，能够更好地理解和预测复杂环境中的动态变化。",
    architecture: "论文提出的架构包含三个核心组件：1. **感知编码器**：使用多模态神经网络处理视觉、触觉和听觉输入，提取环境特征；2. **世界模型**：基于Transformer架构的时空建模模块，预测环境演化；3. **动作解码器**：将高层意图转换为具体的动作指令，支持连续和离散动作。该架构的优势在于端到端的训练方式，避免了传统方法中模块间的不一致性，同时保持了各模块的可解释性。",
    innovations: "1. 提出了统一的时空世界模型框架，同时处理短期感知和长期预测；2. 引入了注意力机制来处理环境中的关键对象和动态变化；3. 在多个具身智能基准测试上取得了state-of-the-art的结果。",
    inspiration: "1. 该架构可以应用于VLA（视觉语言动作）模型的训练，提高模型的多模态理解能力；2. 世界模型的训练策略对于需要长期推理的机器人任务具有重要参考价值；3. 注意力机制的设计思路可以借鉴到其他具身智能系统中，提升处理复杂环境的能力。",
  ...
  },
  {
    ...
    date: "2026-06-04",
    category: "vla",
    summary: "VLA: Visual-Language-Action Model for Robotics Foundation Models提出了一种创新的方法来解决具身智能领域的核心挑战。我们的模型实现了视觉感知、语言理解和动作生成的统一框架，在多个基准测试上取得了state-of-the-art的性能，显示出跨不同机器人平台和环境的强大泛化能力。",
    background: "当前具身智能系统面临着环境理解、长期规划和动作生成的挑战。本文针对这些问题，提出了一种新的世界模型架构，能够更好地理解和预测复杂环境中的动态变化。",
    architecture: "论文提出的架构包含三个核心组件：1. **感知编码器**：使用多模态神经网络处理视觉、触觉和听觉输入，提取环境特征；2. **世界模型**：基于Transformer架构的时空建模模块，预测环境演化；3. **动作解码器**：将高层意图转换为具体的动作指令，支持连续和离散动作。该架构的优势在于端到端的训练方式，避免了传统方法中模块间的不一致性，同时保持了各模块的可解释性。",
    innovations: "1. 提出了统一的时空世界模型框架，同时处理短期感知和长期预测；2. 引入了注意力机制来处理环境中的关键对象和动态变化；3. 在多个具身智能基准测试上取得了state-of-the-art的结果。",
    inspiration: "1. 该架构可以应用于VLA（视觉语言动作）模型的训练，提高模型的多模态理解能力；2. 世界模型的训练策略对于需要长期推理的机器人任务具有重要参考价值；3. 注意力机制的设计思路可以借鉴到其他具身智能系统中，提升处理复杂环境的能力。",
  ...
  },
  {
    ...
    date: "2026-06-04",
    category: "worldmodel",
    summary: "Sim2Real Transfer for End-to-End Robotics Control提出了一种创新的方法来解决具身智能领域的核心挑战。该工作解决了从仿真到现实世界机器人系统的策略迁移挑战，提出了一种新的域适应技术，使用对抗性训练来弥合仿真到现实的差距。",
    background: "当前具身智能系统面临着环境理解、长期规划和动作生成的挑战。本文针对这些问题，提出了一种新的世界模型架构，能够更好地理解和预测复杂环境中的动态变化。",
    architecture: "论文提出的架构包含三个核心组件：1. **感知编码器**：使用多模态神经网络处理视觉、触觉和听觉输入，提取环境特征；2. **世界模型**：基于Transformer架构的时空建模模块，预测环境演化；3. **动作解码器**：将高层意图转换为具体的动作指令，支持连续和离散动作。该架构的优势在于端到端的训练方式，避免了传统方法中模块间的不一致性，同时保持了各模块的可解释性。",
    innovations: "1. 提出了统一的时空世界模型框架，同时处理短期感知和长期预测；2. 引入了注意力机制来处理环境中的关键对象和动态变化；3. 在多个具身智能基准测试上取得了state-of-the-art的结果。",
    inspiration: "1. 该架构可以应用于VLA（视觉语言动作）模型的训练，提高模型的多模态理解能力；2. 世界模型的训练策略对于需要长期推理的机器人任务具有重要参考价值；3. 注意力机制的设计思路可以借鉴到其他具身智能系统中，提升处理复杂环境的能力。",
  ...
  },
  {
    ...
    date: "2026-06-04",
    category: "worldmodel",
    summary: "Foundation Models for Autonomous Navigation in Dynamic Environments提出了一种创新的方法来解决具身智能领域的核心挑战。我们开发了一种基础模型方法用于高度动态环境中的自主导航。我们的模型结合了深度强化学习和几何推理来处理具有移动障碍物和变化环境的复杂场景。",
    background: "当前具身智能系统面临着环境理解、长期规划和动作生成的挑战。本文针对这些问题，提出了一种新的世界模型架构，能够更好地理解和预测复杂环境中的动态变化。",
    architecture: "论文提出的架构包含三个核心组件：1. **感知编码器**：使用多模态神经网络处理视觉、触觉和听觉输入，提取环境特征；2. **世界模型**：基于Transformer架构的时空建模模块，预测环境演化；3. **动作解码器**：将高层意图转换为具体的动作指令，支持连续和离散动作。该架构的优势在于端到端的训练方式，避免了传统方法中模块间的不一致性，同时保持了各模块的可解释性。",
    innovations: "1. 提出了统一的时空世界模型框架，同时处理短期感知和长期预测；2. 引入了注意力机制来处理环境中的关键对象和动态变化；3. 在多个具身智能基准测试上取得了state-of-the-art的结果。",
    inspiration: "1. 该架构可以应用于VLA（视觉语言动作）模型的训练，提高模型的多模态理解能力；2. 世界模型的训练策略对于需要长期推理的机器人任务具有重要参考价值；3. 注意力机制的设计思路可以借鉴到其他具身智能系统中，提升处理复杂环境的能力。",
  ...
  },
  {
    ...
    date: "2026-06-04",
    category: "embodied",
    summary: "Multi-Modal Learning for Human-Robot Interaction提出了一种创新的方法来解决具身智能领域的核心挑战。本文探索了用于改善人机交互的多模态学习方法。我们的框架整合了视觉、语言和触觉信息，创造出更直观和响应性更强的机器人系统。",
    background: "当前具身智能系统面临着环境理解、长期规划和动作生成的挑战。本文针对这些问题，提出了一种新的世界模型架构，能够更好地理解和预测复杂环境中的动态变化。",
    architecture: "论文提出的架构包含三个核心组件：1. **感知编码器**：使用多模态神经网络处理视觉、触觉和听觉输入，提取环境特征；2. **世界模型**：基于Transformer架构的时空建模模块，预测环境演化；3. **动作解码器**：将高层意图转换为具体的动作指令，支持连续和离散动作。该架构的优势在于端到端的训练方式，避免了传统方法中模块间的不一致性，同时保持了各模块的可解释性。",
    innovations: "1. 提出了统一的时空世界模型框架，同时处理短期感知和长期预测；2. 引入了注意力机制来处理环境中的关键对象和动态变化；3. 在多个具身智能基准测试上取得了state-of-the-art的结果。",
    inspiration: "1. 该架构可以应用于VLA（视觉语言动作）模型的训练，提高模型的多模态理解能力；2. 世界模型的训练策略对于需要长期推理的机器人任务具有重要参考价值；3. 注意力机制的设计思路可以借鉴到其他具身智能系统中，提升处理复杂环境的能力。",
  ...
  },
  {
    id: "paper-26030280",
    title: "Qwen-VLA: Unifying Vision-Language-Action Modeling across Tasks, Environments, and Robot Embodiments",
    authors: "Qiuyue Wang, Mingsheng Li, Jian Guan, Jinhui Ye, et al.",
    institution: "[需确认是否为一线机构]",
    date: "2026-05-28",
    category: "embodied",
    summary: "Qwen-VLA扩展Qwen视觉语言建模栈，通过基于DiT的动作解码器实现跨任务、环境和机器人具身的统一视觉-语言-动作建模",
    background: "具身智能通常通过针对个别任务的专门模型研究，导致能力碎片化和有限泛化能力。Qwen-VLA旨在研究异构具身决策问题能否在单一视觉-语言-动作模型中统一。",
    architecture: "统一建模栈从感知、理解和推理扩展到连续动作和轨迹生成；具身感知提示调节支持多种机器人平台；动作和轨迹预测框架实现跨机器人形态的任务迁移。",
    innovations: "统一处理不同具身决策问题的单一视觉-语言-动作模型；大规模联合预训练涵盖多样化的数据源；具身感知提示调节使模型能够适应不同机器人平台。",
    inspiration: "统一的视觉-语言-动作模型框架能有效处理多种具身任务；多样化的训练数据对提高泛化能力至关重要；在DOMINO动态操作任务上实现26.6%的零样本成功率，展现了强大零样本能力。",
    pdfUrl: "https://arxiv.org/pdf/2605.30280",
    htmlUrl: "https://arxiv.org/abs/2605.30280",
    figures: []
  },
  {
    ...
    title: "DIAL: Decoupling Intent and Action via Latent World Modeling",
    authors: "Yi et al.",
    institution: "[待确认一线研究机构]",
    date: "2026-03-29",
    category: "embodied",
    summary: "DIAL通过引入潜在世界建模桥接高层决策和底层执行，实现端到端VLA模型的10倍数据效率提升",
    background: "现有端到端VLA模型通常将VLM作为多模态编码器直接预测连续动作，存在语义与控制鸿沟。双系统架构虽已成趋势，但往往缺乏物理前兆。",
    architecture: "生物启发的双系统架构：系统2（大脑）使用预训练VLM合成潜在意图；系统1（小脑）作为潜在逆动力学模型将观察与预测意图比较推断动作命令。",
    innovations: "通过可微分潜在意图瓶颈实现认知决策和运动执行的结构分离；目标导向视觉前兆使VLM从被动编码器转变为主动决策者；端到端可微分优化防止后崩溃。",
    inspiration: "双系统架构为处理语义-控制鸿沟提供有效解决方案；10倍数据效率提升对资源受限的机器人训练意义重大；在新物体和复杂配置上展现强大零样本泛化能力。",
    pdfUrl: "https://arxiv.org/pdf/2603.29844",
    htmlUrl: "https://arxiv.org/abs/2603.29844",
    figures: []
  },
  {
    ...
    date: "2026-02-06",
    category: "embodied",
    summary: "World-VLA-Loop通过视频世界模型与VLA策略的闭环联合优化，实现真实世界任务成功率36.7%的提升",
    background: "具身AI兴起推动基于连续机器人动作的视频世界模型发展。现有方法往往依赖外部VLM或启发式代理奖励，缺乏RL所需的精确性。现有视频模拟器存在不精确动作跟随问题。",
    architecture: "状态感知视频世界模型基于Cosmos-Predict 2构建，使用Diffusion Transformer联合预测未来观察和奖励信号；闭环优化框架实现世界模型和策略的共进化。",
    innovations: "建立世界模型和VLA策略间的共进化循环；引入SANS数据集包含近成功轨迹；世界模型达到87.9%视觉对齐率和86.4%奖励对齐率，成为高保真模拟器。",
    inspiration: "闭环联合优化显著提升性能，超过单独优化方法；通过少量物理交互实现显著性能提升；迭代改进从13.3%基础成功率到36.7%最终成功率。",
    pdfUrl: "https://arxiv.org/pdf/2602.06508",
    htmlUrl: "https://arxiv.org/abs/2602.06508",
    figures: []
  },
  {
    id: "paper-251016732",
