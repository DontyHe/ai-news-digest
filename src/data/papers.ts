// 论文数据 - 由 cron 任务每日更新
// 更新路径: /home/Hebin/project/ai-news-digest/src/data/papers.json

export interface Paper {
  id: string;
  title: string;
  authors: string;
  date: string;
  institution: string;
  category: "embodied" | "vla" | "worldmodel" | "ai";
  summary: string;
  background: string;
  architecture: string;
  innovations: string;
  inspiration: string;
  pdfUrl: string;
  htmlUrl: string;
  figures: string[];
}

export const papers: Paper[] = [
  {
    id: "paper-260311041",
    title: "DynVLA: Learning World Dynamics for Action Reasoning in Autonomous Driving",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 1. DynVLA: Learning World Dynamics for Action Reasoning in Autonomous Driving  - **论文ID**: arXiv:2603.11041 - **作者**: Shuyao Shang, Bing Zhan, Yunfei Yan, Yuqi Wang, Yingyan Li, Yasong An, Xiaoman ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.11041",
    htmlUrl: "https://arxiv.org/abs/2603.11041",
    figures: []
  },
  {
    id: "paper-260310712",
    title: "FutureVLA: Joint Visuomotor Prediction for Vision-Language-Action Model",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 2. FutureVLA: Joint Visuomotor Prediction for Vision-Language-Action Model  - **论文ID**: arXiv:2603.10712 - **作者**: Xiaoxu Xu, Hao Li, Jinhui Ye, Yilun Chen, Jia Zeng, Xinyi Chen, Linning Xu, Dahua ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10712",
    htmlUrl: "https://arxiv.org/abs/2603.10712",
    figures: []
  },
  {
    id: "paper-260310448",
    title: "DiT4DiT: Jointly Modeling Video Dynamics and Actions for Generalizable Robot Control",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 3. DiT4DiT: Jointly Modeling Video Dynamics and Actions for Generalizable Robot Control  - **论文ID**: arXiv:2603.10448 - **作者**: Teli Ma, Jia Zheng, Zifan Wang, Chuili Jiang, Andy Cui, Junwei Liang,",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10448",
    htmlUrl: "https://arxiv.org/abs/2603.10448",
    figures: []
  },
  {
    id: "paper-260310980",
    title: "PPGuide: Steering Diffusion Policies with Performance Predictive Guidance",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 4. PPGuide: Steering Diffusion Policies with Performance Predictive Guidance  - **论文ID**: arXiv:2603.10980 - **作者**: Zixing Wang, Devesh K. Jha, Ahmed H. Qureshi, Diego Romeres - **机构**: 未知 - **发布日",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10980",
    htmlUrl: "https://arxiv.org/abs/2603.10980",
    figures: []
  },
  {
    id: "paper-260310971",
    title: "Contact Coverage-Guided Exploration for General-Purpose Dexterous Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 5. Contact Coverage-Guided Exploration for General-Purpose Dexterous Manipulation  - **论文ID**: arXiv:2603.10971 - **作者**: Zixuan Liu, Ruoyi Qiao, Chenrui Tie, Xuanwei Liu, Yunfan Lou, Chongkai Gao,",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10971",
    htmlUrl: "https://arxiv.org/abs/2603.10971",
    figures: []
  },
  {
    id: "paper-260310469",
    title: "DepthCache: Depth-Guided Training-Free Visual Token Merging for VLA Inference",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 6. DepthCache: Depth-Guided Training-Free Visual Token Merging for VLA Inference  - **论文ID**: arXiv:2603.10469 - **作者**: Yuquan Li, Lianjie Ma, Han Ding, Lijun Zhu - **机构**: 未知 - **发布日期**: 2026-03-",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10469",
    htmlUrl: "https://arxiv.org/abs/2603.10469",
    figures: []
  },
  {
    id: "paper-260310871",
    title: "FG-CLTP: Fine-Grained Contrastive Language Tactile Pretraining",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 7. FG-CLTP: Fine-Grained Contrastive Language Tactile Pretraining  - **论文ID**: arXiv:2603.10871 - **作者**: Wenxuan Ma, Chaofan Zhang, Yinghao Cai, Guocai Yao, Shaowei Cui, Shuo Wang - **机构**: 未知 - *",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10871",
    htmlUrl: "https://arxiv.org/abs/2603.10871",
    figures: []
  },
  {
    id: "paper-260310340",
    title: "Overcoming Visual Clutter in Vision Language Action Models via Concept-Gated Visual Distillation",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 8. Overcoming Visual Clutter in Vision Language Action Models via Concept-Gated Visual Distillation  - **论文ID**: arXiv:2603.10340 - **作者**: Sangmim Song, Sarath Kodagoda, Marc Carmichael, Karthick ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10340",
    htmlUrl: "https://arxiv.org/abs/2603.10340",
    figures: []
  },
  {
    id: "paper-260310441",
    title: "KnowDiffuser: A Knowledge-Guided Diffusion Planner",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 9. KnowDiffuser: A Knowledge-Guided Diffusion Planner  - **论文ID**: arXiv:2603.10441 - **作者**: Fan Ding, Xuewen Luo, Fengze Yang, Bo Yu, HwaHui Tew, Ganesh Krishnasami, Junn Yong Loo - **机构**: 未知 - ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10441",
    htmlUrl: "https://arxiv.org/abs/2603.10441",
    figures: []
  },
  {
    id: "paper-260310675",
    title: "Cybo-Waiter: A Physical Agentic Framework for Humanoid Whole-Body Locomotion-Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-12",
    category: "embodied",
    summary: "## 10. Cybo-Waiter: A Physical Agentic Framework for Humanoid Whole-Body Locomotion-Manipulation  - **论文ID**: arXiv:2603.10675 - **作者**: Peng Ren, Haoyang Ge, Chuan Qi, Cong Huang, Hong Li, Jiang Zhao",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.10675",
    htmlUrl: "https://arxiv.org/abs/2603.10675",
    figures: []
  },
  {
    id: "paper-260309971",
    title: "TiPToP: A Modular Open-Vocabulary Planning System for Robotic Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-11",
    category: "embodied",
    summary: "## 1. TiPToP: A Modular Open-Vocabulary Planning System for Robotic Manipulation  - **论文ID**: arXiv:2603.09971 - **作者**: William Shen, Nishanth Kumar, Sahit Chintalapudi, Jie Wang, Christopher Watson,",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.09971",
    htmlUrl: "https://arxiv.org/abs/2603.09971",
    figures: []
  },
  {
    id: "paper-260309882",
    title: "Emerging Extrinsic Dexterity in Cluttered Scenes via Dynamics-aware Policy Learning",
    authors: "et al.",
    institution: "",
    date: "2026-03-11",
    category: "embodied",
    summary: "## 2. Emerging Extrinsic Dexterity in Cluttered Scenes via Dynamics-aware Policy Learning  - **论文ID**: arXiv:2603.09882 - **作者**: Yixin Zheng, Jiangran Lyu, Yifan Zhang, Jiayi Chen, Mi Yan, Yuntian De",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.09882",
    htmlUrl: "https://arxiv.org/abs/2603.09882",
    figures: []
  },
  {
    id: "paper-260309712",
    title: "Robotic Scene Cloning: Advancing Zero-Shot Robotic Scene Adaptation in Manipulation via Visual Prompt Editing",
    authors: "et al.",
    institution: "",
    date: "2026-03-11",
    category: "embodied",
    summary: "## 3. Robotic Scene Cloning: Advancing Zero-Shot Robotic Scene Adaptation in Manipulation via Visual Prompt Editing  - **论文ID**: arXiv:2603.09712 - **作者**: Binyuan Huang, Yuqing Wen, Yucheng Zhao, Yao",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.09712",
    htmlUrl: "https://arxiv.org/abs/2603.09712",
    figures: []
  },
  {
    id: "paper-260309961",
    title: "BEACON: Language-Conditioned Navigation Affordance Prediction under Occlusion",
    authors: "et al.",
    institution: "",
    date: "2026-03-11",
    category: "embodied",
    summary: "## 4. BEACON: Language-Conditioned Navigation Affordance Prediction under Occlusion  - **论文ID**: arXiv:2603.09961 - **作者**: (待查证) - **机构**: (待查证) - **发布日期**: 2026-03-10 - **类别**: cs.RO - **链接**: [PDF]",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.09961",
    htmlUrl: "https://arxiv.org/abs/2603.09961",
    figures: []
  },
  {
    id: "planning-in-8-tokens-2026-03-07",
    title: "Planning in 8 Tokens: A Compact Discrete Tokenizer for Latent World Model",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 1. Planning in 8 Tokens: A Compact Discrete Tokenizer for Latent World Model  - **作者**: Dongwon Kim, Gawon Seo, Jinsung Lee, Minsu Cho, Suha Kwak - **机构**: KAIST (韩国科学技术院) - **发布日期**: 2026-03-05 -",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "physiflow-2026-03-07",
    title: "PhysiFlow: Physics-Aware Humanoid Whole-Body VLA via Multi-Brain Latent Flow Matching and Robust Tracking",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 2. PhysiFlow: Physics-Aware Humanoid Whole-Body VLA via Multi-Brain Latent Flow Matching and Robust Tracking  - **作者**: Weikai Qin, Sichen Wu, Ci Chen, Mengfan Liu, Linxi Feng - **机构**:  Carnegie ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "critic-in-the-loop-2026-03-07",
    title: "Critic in the Loop: A Tri-System VLA Framework for Robust Long-Horizon Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 3. Critic in the Loop: A Tri-System VLA Framework for Robust Long-Horizon Manipulation  - **作者**: Pengfei Yi, Yingjie Ma, Wenjiang Xu, Yanan Hao, Shuai Gan - **机构**: 上海交通大学 - **发布日期**: 2026-03-05 ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "observing-and-controlling-2026-03-07",
    title: "Observing and Controlling Features in Vision-Language-Action Models",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 4. Observing and Controlling Features in Vision-Language-Action Models  - **作者**: Hugo Buurmeijer, Carmen Amo Alonso, Aiden Swann, Marco Pavone - **机构**: Stanford University - **发布日期**: 2026-03-05",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "robopocket-2026-03-07",
    title: "RoboPocket: Improve Robot Policies Instantly with Your Phone",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 5. RoboPocket: Improve Robot Policies Instantly with Your Phone  - **作者**: Junjie Fang, Wendi Chen, Han Xue, Fangyuan Zhou, Tian Le - **机构**: 未明确（待查证） - **发布日期**: 2026-03-05 - **类别**: cs.RO - **论文",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "curobov2-2026-03-07",
    title: "cuRoboV2: Dynamics-Aware Motion Generation with Depth-Fused Distance Fields for High-DoF Robots",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 6. cuRoboV2: Dynamics-Aware Motion Generation with Depth-Fused Distance Fields for High-DoF Robots  - **作者**: Balakumar Sundaralingam, Adithyavairavan Murali, Stan Birchfield - **机构**: NVIDIA - **",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "ultradexgrasp-2026-03-07",
    title: "UltraDexGrasp: Learning Universal Dexterous Grasping for Bimanual Robots with Synthetic Data",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 7. UltraDexGrasp: Learning Universal Dexterous Grasping for Bimanual Robots with Synthetic Data  - **作者**: Sizhe Yang, Yiman Xie, Zhixuan Liang, Yang Tian, Jia Zeng - **机构**:  Zhejiang University ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "omni-manip-2026-03-07",
    title: "Omni-Manip: Beyond-FOV Large-Workspace Humanoid Manipulation with Omnidirectional 3D Perception",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 8. Omni-Manip: Beyond-FOV Large-Workspace Humanoid Manipulation with Omnidirectional 3D Perception  - **作者**: Pei Qu, Zheng Li, Yufei Jia, Ziyun Liu, Liang Zhu - **机构**: 清华大学 - **发布日期**: 2026-03-0",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "latent-policy-steering-th-2026-03-07",
    title: "Latent Policy Steering through One-Step Flow Policies",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 9. Latent Policy Steering through One-Step Flow Policies  - **作者**: Hokyun Im, Andrey Kolobov, Jianlong Fu, Youngwoon Lee - **机构**:  University of Southern California (USC) - **发布日期**: 2026-03-05 ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "openfrontier-2026-03-07",
    title: "OpenFrontier: General Navigation with Visual-Language Grounded Frontiers",
    authors: "et al.",
    institution: "",
    date: "2026-03-07",
    category: "embodied",
    summary: "### 10. OpenFrontier: General Navigation with Visual-Language Grounded Frontiers  - **作者**: Esteban Padilla, Boyang Sun, Marc Pollefeys, Hermann Blum - **机构**: ETH Zurich - **发布日期**: 2026-03-05 - **类别",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "physgen-2026-03-06",
    title: "PhysGen: Learning Physics from Pretrained Video Models",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 1. PhysGen: Learning Physics from Pretrained Video Models  **作者**: Zijian Song, Qichang Li, Sihan Qin, Yuhao Chen, Tianshui Chen, Liang Lin, Guangrun Wang   **机构**: 疑似中山大学/北京大学   **日期**: 2026-02-1",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "streamvla-2026-03-06",
    title: "StreamVLA: Breaking the Reason-Act Cycle via Completion-State Gating",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 2. StreamVLA: Breaking the Reason-Act Cycle via Completion-State Gating  **作者**: Tongqing Chen, Hang Wu, Jiasen Wang, Xiaotao Li, Lu Fang   **日期**: 2026-02-07   **PDF**: https://arxiv.org/pdf/2602",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "robmrag-2026-03-06",
    title: "RobMRAG: Zero-Shot Robotic Manipulation via 3D Gaussian Splatting-Enhanced MRAG",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 3. RobMRAG: Zero-Shot Robotic Manipulation via 3D Gaussian Splatting-Enhanced MRAG  **作者**: Zilong Xie, Jingyu Gong, Xin Tan, Zhizhong Zhang, Yuan Xie   **日期**: 2026-02-28   **PDF**: https://arxiv",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "i-perceive-2026-03-06",
    title: "I-Perceive: A Foundation Model for Active Perception with Language Instructions",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 4. I-Perceive: A Foundation Model for Active Perception with Language Instructions  **作者**: Yongxi Huang, Zhuohang Wang, Wenjing Tang, Cewu Lu, Panpan Cai   **机构**: 上海交通大学   **日期**: 2026-02-28   *",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "inject-once-survive-later-2026-03-06",
    title: "Inject Once Survive Later: Backdooring Vision-Language-Action Models",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 5. Inject Once Survive Later: Backdooring Vision-Language-Action Models  **作者**: Jianyi Zhou, Yujie Wei, Ruichen Zhen, Bo Zhao, Xiaobo Xia, Rui Shao, Xiu Su, Shuo Yang   **日期**: 2026-01-31   **PDF",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "simpletool-2026-03-06",
    title: "SimpleTool: Parallel Decoding for Real-Time LLM Function Calling",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 6. SimpleTool: Parallel Decoding for Real-Time LLM Function Calling  **作者**: Xiaoxin Shi, Jiaxin Wan, Linkang Dong, Wei Jiang, Yue Liu, Zengfeng Huang   **日期**: 2026-02-04   **PDF**: https://arxiv",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "rethinking-the-role-of-en-2026-03-06",
    title: "Rethinking the Role of Entropy in Optimizing Tool-Use Behaviors",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 7. Rethinking the Role of Entropy in Optimizing Tool-Use Behaviors  **作者**: Zeping Li, Hongru Wang, Yiwen Zhao, Guanhua Chen, Yixia Li, Keyang Chen, Yixin Cao, Guangnan Ye, Hongfeng Chai, Zhenfei ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "context-learning-for-mult-2026-03-06",
    title: "Context Learning for Multi-Agent Discussion",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 8. Context Learning for Multi-Agent Discussion  **作者**: Xingyuan Hua, Sheng Yue, Xinyi Li, Yizhe Zhao, Jinrui Zhang, Ju Ren   **日期**: 2026-02-02   **PDF**: https://arxiv.org/pdf/2602.02350   **HTM",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "a-user-study-on-teleopera-2026-03-06",
    title: "A User Study on Teleoperation Interfaces for Primitive Manipulation Tasks",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 9. A User Study on Teleoperation Interfaces for Primitive Manipulation Tasks  **作者**: Jun Aoki, Shunki Itadera   **日期**: 2026-02-03   **PDF**: https://arxiv.org/pdf/2603.00020   **HTML**: https://",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "tilt-ropter-2026-03-06",
    title: "Tilt-Ropter: Hybrid Aerial and Terrestrial Vehicle with Tilt Rotors",
    authors: "et al.",
    institution: "",
    date: "2026-03-06",
    category: "embodied",
    summary: "### 10. Tilt-Ropter: Hybrid Aerial and Terrestrial Vehicle with Tilt Rotors  **作者**: Ruoyi Wang, Xuchen Liu, Zongzhou Wu, Zixuan Guo, Wendi Ding, Ben M. Chen   **日期**: 2026-02-02   **PDF**: https://ar",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "chain-of-world-2026-03-05",
    title: "Chain of World: World Model Thinking in Latent Motion",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 1. Chain of World: World Model Thinking in Latent Motion  **作者**: Fuxiang Yang, Donglin Di, Lulu Tang, Xuancheng Zhang, Lei Fan, Hao Li, Chen Wei, Tonghua Su, Baorui Ma   **机构**: 旷视科技 (Megvii)   **",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "ace-brain-0-2026-03-05",
    title: "ACE-Brain-0: Spatial Intelligence as a Shared Scaffold for Universal Embodiments",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 2. ACE-Brain-0: Spatial Intelligence as a Shared Scaffold for Universal Embodiments  **作者**: Ziyang Gong, Zehang Luo, Anke Tang, Zhe Liu, Shi Fu, Zhi Hou, Ganlin Lin, Weiyun Wang, Xiaofeng Wang, Ji",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "joint-aligned-latent-acti-2026-03-05",
    title: "Joint-Aligned Latent Action: Towards Scalable VLA Pretraining in the Wild",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 3. Joint-Aligned Latent Action: Towards Scalable VLA Pretraining in the Wild  **作者**: Hao Luo, Ye Wang, Wanpeng Zhang, Haoqi Yuan, Yicheng Feng, Haiweng Xu, Sipeng Zheng, Zongqing Lu   **机构**: 北大, ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "universal-pose-pretrainin-2026-03-05",
    title: "Universal Pose Pretraining for Generalizable Vision-Language-Action Policies",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 4. Universal Pose Pretraining for Generalizable Vision-Language-Action Policies  **作者**: Haitao Lin, Hanyang Yu, Jingshun Zhang, He Zhang, Yonggen Ling, Ping Tan, Xiangyang Xue, Yanwei Fu   **机构**:",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "tether-2026-03-05",
    title: "Tether: Autonomous Functional Play with Correspondence-Driven Trajectory Warping",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 5. Tether: Autonomous Functional Play with Correspondence-Driven Trajectory Warping  **作者**: William Liang, Sam Wang, Hung-Ju Wang, Osbert Bastani, Yecheng Jason Ma, Dinesh Jayaraman   **机构**: UPen",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "utonia-2026-03-05",
    title: "Utonia: Toward One Encoder for All Point Clouds",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 6. Utonia: Toward One Encoder for All Point Clouds  **作者**: Yujia Zhang, Xiaoyang Wu, Yunhan Yang, Xianzhe Fan, Han Li, Yuechen Zhang, Zehao Huang, Naiyan Wang, Hengshuang Zhao   **机构**: 港中文, 上海AI ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "cmoe-2026-03-05",
    title: "CMoE: Contrastive Mixture of Experts for Motion Control and Terrain Adaptation of Humanoid Robots",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 7. CMoE: Contrastive Mixture of Experts for Motion Control and Terrain Adaptation of Humanoid Robots  **作者**: Shihao Ma, Hongjin Chen, Zijun Xu, Yi Zhao, Ke Wu, Ruichen Yang, Leyao Zou, Zhongxue Di",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "ma-conav-2026-03-05",
    title: "MA-CoNav: A Master-Slave Multi-Agent Framework with Hierarchical Collaboration and Dual-Level Reflection for Long-Horizon Embodied VLN",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 8. MA-CoNav: A Master-Slave Multi-Agent Framework with Hierarchical Collaboration and Dual-Level Reflection for Long-Horizon Embodied VLN  **作者**: Ling Luo, Qianqian Bai   **机构**: 哈尔滨工业大学   **发布日期*",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "adm-dp-2026-03-05",
    title: "ADM-DP: Adaptive Dynamic Modality Diffusion Policy through Vision-Tactile-Graph Fusion for Multi-Agent Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 9. ADM-DP: Adaptive Dynamic Modality Diffusion Policy through Vision-Tactile-Graph Fusion for Multi-Agent Manipulation  **作者**: Enyi Wang, Wen Fan, Dandan Zhang   **机构**: 清华大学   **发布日期**: 2026/02/2",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "denoising-particle-filter-2026-03-05",
    title: "Denoising Particle Filters: Learning State Estimation with Single-Step Objectives",
    authors: "et al.",
    institution: "",
    date: "2026-03-05",
    category: "embodied",
    summary: "## 10. Denoising Particle Filters: Learning State Estimation with Single-Step Objectives  **作者**: Lennart Röstel, Berthold Bäuml   **机构**: TUM (慕尼黑工业大学)   **发布日期**: 2026/02/23   **arXiv ID**: 2602.196",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "paper-260303283",
    title: "Utonia: Toward One Encoder for All Point Clouds",
    authors: "et al.",
    institution: "",
    date: "2026-03-04",
    category: "embodied",
    summary: "## 1. Utonia: Toward One Encoder for All Point Clouds  - **论文ID**: arXiv:2603.03283 - **作者**: Yujia Zhang, Xiaoyang Wu, Yunhan Yang, Xianzhe Fan, Han Li, Yuechen Zhang, Zehao Huang, Naiyan Wang, Hengs",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.03283",
    htmlUrl: "https://arxiv.org/abs/2603.03283",
    figures: []
  },
  {
    id: "paper-260303282",
    title: "MIBURI: Towards Expressive Interactive Gesture Synthesis",
    authors: "et al.",
    institution: "",
    date: "2026-03-04",
    category: "embodied",
    summary: "## 2. MIBURI: Towards Expressive Interactive Gesture Synthesis  - **论文ID**: arXiv:2603.03282 - **作者**: M. Hamza Mughal, Rishabh Dabral, Vera Demberg, Christian Theobalt - **机构**: MPI Informatics, TU D",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.03282",
    htmlUrl: "https://arxiv.org/abs/2603.03282",
    figures: []
  },
  {
    id: "paper-260303278",
    title: "Tether: Autonomous Functional Play with Correspondence-Driven Trajectory Warping",
    authors: "et al.",
    institution: "",
    date: "2026-03-04",
    category: "embodied",
    summary: "## 3. Tether: Autonomous Functional Play with Correspondence-Driven Trajectory Warping  - **论文ID**: arXiv:2603.03278 - **作者**: William Liang, Sam Wang, Hung-Ju Wang, Osbert Bastani, Yecheng Jason Ma, ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.03278",
    htmlUrl: "https://arxiv.org/abs/2603.03278",
    figures: []
  },
  {
    id: "paper-260303279",
    title: "ULTRA: Unified Multimodal Control for Autonomous Humanoid Whole-Body Loco-Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-04",
    category: "embodied",
    summary: "## 4. ULTRA: Unified Multimodal Control for Autonomous Humanoid Whole-Body Loco-Manipulation  - **论文ID**: arXiv:2603.03279 - **作者**: Xialin He, Sirui Li, Xinyao Yao, Runpei Dong, Liuyu Bian, Yu-Xiong ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.03279",
    htmlUrl: "https://arxiv.org/abs/2603.03279",
    figures: []
  },
  {
    id: "paper-260303243",
    title: "HoMMI: Learning Whole-Body Mobile Manipulation from Human Demonstrations",
    authors: "et al.",
    institution: "",
    date: "2026-03-04",
    category: "embodied",
    summary: "## 5. HoMMI: Learning Whole-Body Mobile Manipulation from Human Demonstrations  - **论文ID**: arXiv:2603.03243 - **作者**: Xiaomeng Xu, Jisang Park, Han Zhang, Eric Cousineau, Aditya Bhat, Jose Barreiros,",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "https://arxiv.org/pdf/2603.03243",
    htmlUrl: "https://arxiv.org/abs/2603.03243",
    figures: []
  },
  {
    id: "robust-skills,-brittle-gr-2026-03-03",
    title: "Robust Skills, Brittle Grounding: Diagnosing Restricted Generalization in Vision-Language Action Policies via Multi-Object Picking",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 1. Robust Skills, Brittle Grounding: Diagnosing Restricted Generalization in Vision-Language Action Policies via Multi-Object Picking  - **作者**: David Emukpere, Romain Deffayet, Jean-Michel Render",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "planning-from-observation-2026-03-03",
    title: "Planning from Observation and Interaction",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 2. Planning from Observation and Interaction  - **作者**: Tyler Han, Siyang Shen, Rohan Baijal, Harine Ravichandiran, Bat Nemekhbold, Kevin Huang, Sanghun Jung, Byron Boots - **机构**: University of W",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "foundation-world-models-f-2026-03-03",
    title: "Foundation World Models for Agents that Learn, Verify, and Adapt Reliably Beyond Static Environments",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 3. Foundation World Models for Agents that Learn, Verify, and Adapt Reliably Beyond Static Environments  - **作者**: Florent Delgrange - **机构**: 未公开 - **发布日期**: 2026-02-27 - **类别**: cs.LG, cs.AI - *",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "stemvla-2026-03-03",
    title: "StemVLA: An Open-Source Vision-Language-Action Model with Future 3D Spatial Geometry Knowledge and 4D Historical Representation",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 4. StemVLA: An Open-Source Vision-Language-Action Model with Future 3D Spatial Geometry Knowledge and 4D Historical Representation  - **作者**: Jiasong Xiao, Yutao She, Kai Li, Yuyang Sha, Ziang Che",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "favla-2026-03-03",
    title: "FAVLA: A Force-Adaptive Fast-Slow VLA model for Contact-Rich Robotic Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 5. FAVLA: A Force-Adaptive Fast-Slow VLA model for Contact-Rich Robotic Manipulation  - **作者**: Yao Li, Peiyuan Tang, Wuyang Zhang, Chengyang Zhu, Yifan Duan, Weikai Shi, Xiaodong Zhang, Zijiang Y",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "vca-2026-03-03",
    title: "VCA: Vision-Click-Action Framework for Precise Manipulation of Segmented Objects in Target Ambiguous Environments",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 6. VCA: Vision-Click-Action Framework for Precise Manipulation of Segmented Objects in Target Ambiguous Environments  - **作者**: Donggeon Kim, Seungwon Jan, Hyeonjun Park, Daegyu Lim - **机构**: 未公开 ",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "dysl-vla-2026-03-03",
    title: "DySL-VLA: Efficient Vision-Language-Action Model Inference via Dynamic-Static Layer-Skipping for Robot Manipulation",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 7. DySL-VLA: Efficient Vision-Language-Action Model Inference via Dynamic-Static Layer-Skipping for Robot Manipulation  - **作者**: 未详 - **发布日期**: 2026-02-26 - **类别**: cs.RO - **论文链接**: [PDF](https:",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "rethinking-the-practicali-2026-03-03",
    title: "Rethinking the Practicality of Vision-language-action Model: A Comprehensive Benchmark and An Improved Baseline",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 8. Rethinking the Practicality of Vision-language-action Model: A Comprehensive Benchmark and An Improved Baseline  - **作者**: 未详 - **发布日期**: 2026-02-26 - **类别**: cs.RO - **论文链接**: [PDF](https://ar",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "world-guidance-2026-03-03",
    title: "World Guidance: World Modeling in Condition Space for Action Generation",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 9. World Guidance: World Modeling in Condition Space for Action Generation  - **作者**: 未详 - **发布日期**: 2026-02-25 - **类别**: cs.LG - **论文链接**: [PDF](https://arxiv.org/pdf/2602.22567) | [HTML](https:/",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  {
    id: "aoe-2026-03-03",
    title: "AoE: Always-on Egocentric Human Video Collection for Embodied AI",
    authors: "et al.",
    institution: "",
    date: "2026-03-03",
    category: "embodied",
    summary: "### 10. AoE: Always-on Egocentric Human Video Collection for Embodied AI  - **作者**: 未详（大规模团队） - **发布日期**: 2026-02-27 - **类别**: cs.CV - **论文链接**: [PDF](https://arxiv.org/pdf/2602.23893) | [HTML](https:",
    background: "",
    architecture: "",
    innovations: "",
    inspiration: "",
    pdfUrl: "",
    htmlUrl: "",
    figures: []
  },
  // 2026-03-16 新增
  {
    id: "psi-zero-2026-03-16",
    title: "Ψ_0: An Open Foundation Model Towards Universal Humanoid Loco-Manipulation",
    authors: "Songlin Wei, Hongyi Jing, Boqian Li et al.",
    institution: "清华大学、NVIDIA等",
    date: "2026-03-12",
    category: "embodied",
    summary: "提出Ψ_0开源基础模型，通过解耦学习范式解决人形机器人运动操作任务，数据效率较基线提升40%以上。",
    background: "现有方法通常在大规模人类和人形机器人数据上进行联合训练，但存在根本的运动学差异导致数据效率低下",
    architecture: "两阶段训练范式：1) VLM预训练获取可泛化的视觉-动作表示 2) flow-based动作专家微调",
    innovations: "提出解耦学习范式，分离视觉表示学习和精确关节控制；证明高质量egocentric数据优于大规模互联网噪声数据",
    inspiration: "对于VLA模型训练，数据质量比数量更重要。分阶段训练可以更好利用异构数据源",
    pdfUrl: "https://arxiv.org/pdf/2603.12263",
    htmlUrl: "https://arxiv.org/abs/2603.12263",
    figures: []
  },
  {
    id: "sapave-2026-03-16",
    title: "SaPaVe: Towards Active Perception and Manipulation in Vision-Language-Action Models for Robotics",
    authors: "Mengzhen Liu, Enshen Zhou, Cheng Chi et al.",
    institution: "北京大学等",
    date: "2026-03-12",
    category: "vla",
    summary: "提出端到端框架联合学习语义驱动的主动感知与鲁棒视角不变操作执行。",
    background: "现有方法难以统一语义驱动的主动感知与鲁棒视角不变执行，通常将相机和操作动作放在共享动作空间",
    architecture: "解耦设计：将相机控制与操作动作解耦；自底向上训练策略",
    innovations: "提出主动感知与操作联合学习框架；突破视角变化带来的执行鲁棒性问题",
    inspiration: "主动感知与操作解耦可能是提升VLA泛化性的关键",
    pdfUrl: "https://arxiv.org/pdf/2603.12193",
    htmlUrl: "https://arxiv.org/abs/2603.12193",
    figures: []
  },
  {
    id: "simple-recipe-2026-03-16",
    title: "Simple Recipe Works: Vision-Language-Action Models are Natural Continual Learners with Reinforcement Learning",
    authors: "Jiaheng Hu, Jay Shim, Chen Tang et al.",
    institution: "UT Austin, Sony AI等",
    date: "2026-03-12",
    category: "vla",
    summary: "系统研究证明大型预训练VLA模型本身就是自然的持续学习者，无需复杂策略即可避免灾难性遗忘。",
    background: "持续强化学习是实现自改进具身智能体的方向，传统观点认为需要复杂策略避免灾难性遗忘",
    architecture: "系统研究三种模型、五种任务的持续学习表现",
    innovations: "证明简单微调对大模型已足够；提出VLA作为自然持续学习者的洞见",
    inspiration: "对于VLA持续学习，简单的微调可能已经足够，不需要复杂的正则化策略",
    pdfUrl: "https://arxiv.org/pdf/2603.11653",
    htmlUrl: "https://arxiv.org/abs/2603.11653",
    figures: []
  },
  {
    id: "roboclaw-2026-03-16",
    title: "RoboClaw: An Agentic Framework for Scalable Long-Horizon Robotic Tasks",
    authors: "Ruiying Li, Yunlang Zhou, YuYao Zhu et al.",
    institution: "字节跳动等",
    date: "2026-03-12",
    category: "vla",
    summary: "提出统一数据收集、策略学习和任务执行的VLM驱动智能体框架，解决长程任务扩展难题。",
    background: "现有流程将数据收集、策略学习和部署分离，导致严重依赖人工环境重置和多策略执行脆弱",
    architecture: "统一框架：任务规划器+策略执行器+环境重置器",
    innovations: "VLM驱动的端到端长程任务框架",
    inspiration: "统一框架可能是解决长程任务扩展问题的有效路径",
    pdfUrl: "https://arxiv.org/pdf/2603.11558",
    htmlUrl: "https://arxiv.org/abs/2603.11558",
    figures: []
  },
  {
    id: "omnistream-2026-03-16",
    title: "OmniStream: Mastering Perception, Reconstruction and Action in Continuous Streams",
    authors: "Yibin Yan, Jilan Xu, Shangzhe Di et al.",
    institution: "上海AI Lab等",
    date: "2026-03-12",
    category: "worldmodel",
    summary: "提出统一流式视觉骨干，有效处理感知、重建和动作，支持帧级在线处理。",
    background: "当前视觉模型分散在图像语义感知、离线时序建模和空间几何三个方向，缺乏统一处理流式环境的能力",
    architecture: "因果时空注意力+3D旋转位置编码+多任务预训练",
    innovations: "单一骨干同时处理语义、空间和时序推理；冻结骨干在多个任务达SOTA",
    inspiration: "统一视觉骨干可能是具身智能的重要方向",
    pdfUrl: "https://arxiv.org/pdf/2603.12265",
    htmlUrl: "https://arxiv.org/abs/2603.12265",
    figures: []
  },
  {
    id: "svll-2026-03-16",
    title: "SVLL: Staged Vision-Language Learning for Physically Grounded Embodied Task Planning",
    authors: "Yuyuan Yang, Junkun Hong, Hongrong Wang et al.",
    institution: "上海AI Lab等",
    date: "2026-03-12",
    category: "embodied",
    summary: "提出三阶段框架解决具身任务规划中联合训练导致的时序绑定过早问题。",
    background: "现有训练范式面临关键权衡：联合端到端训练导致过早时序绑定，标准RL方法存在优化不稳定问题",
    architecture: "三阶段框架：视觉语言对齐+动作空间构建+策略微调",
    innovations: "解决时序绑定过早与优化不稳定之间的权衡",
    inspiration: "分阶段训练可能是解决具身任务规划训练不稳定的关键",
    pdfUrl: "https://arxiv.org/pdf/2603.11563",
    htmlUrl: "https://arxiv.org/abs/2603.11563",
    figures: []
  },
  {
    id: "radar-2026-03-16",
    title: "RADAR: Closed-Loop Robotic Data Generation via Semantic Planning and Autonomous Causal Environment Reset",
    authors: "Yongzhong Wang, Keyu Zhu, Yong Zhong et al.",
    institution: "复旦大学等",
    date: "2026-03-12",
    category: "embodied",
    summary: "提出全自主闭环数据生成引擎，完全消除人工干预的数据收集瓶颈。",
    background: "大规模物理交互数据获取是现代机器人学习的关键前提，但人类参与的数据收集成本高、扩展性差",
    architecture: "四模块认知分工：语义规划器+场景管理器+动作控制器+因果环境重置器",
    innovations: "全自主闭环数据生成",
    inspiration: "自动化数据收集对机器人学习至关重要",
    pdfUrl: "https://arxiv.org/pdf/2603.11811",
    htmlUrl: "https://arxiv.org/abs/2603.11811",
    figures: []
  },
  {
    id: "dynvla-2026-03-16",
    title: "DynVLA: Learning World Dynamics for Action Reasoning in Autonomous Driving",
    authors: "Shuyao Shang, Bing Zhan, Yunfei Yan et al.",
    institution: "中科院自动化所等",
    date: "2026-03-11",
    category: "worldmodel",
    summary: "提出DynVLA驾驶VLA模型，在动作生成前先预测紧凑世界动态，实现更知情、物理接地的决策。",
    background: "驾驶场景中环境动态丰富，智能体需要理解交互密集场景中的动态演变",
    architecture: "Dynamics CoT + Dynamics Tokenizer + 解耦ego-centric和env-centric动态",
    innovations: "引入CoT范式到驾驶决策；紧凑动态表示学习",
    inspiration: "世界模型可能是提升VLA物理推理能力的关键",
    pdfUrl: "https://arxiv.org/pdf/2603.11041",
    htmlUrl: "https://arxiv.org/abs/2603.11041",
    figures: []
  },
  {
    id: "futurevla-2026-03-16",
    title: "FutureVLA: Joint Visuomotor Prediction for Vision-Language-Action Model",
    authors: "Xiaoxu Xu, Hao Li, Jinhui Ye et al.",
    institution: "上海AI Lab等",
    date: "2026-03-11",
    category: "vla",
    summary: "提出联合视觉运动预测框架，让VLA模型在预测未来时捕获紧密耦合的视觉-运动相互作用。",
    background: "机器人运动执行受限于对环境几何的视觉感知，有效预测未来需要捕获这种紧密耦合的视觉-运动交互",
    architecture: "联合视觉运动预测",
    innovations: "解决显式方法关注任务无关视觉细节、隐式方法缺乏物理基础的问题",
    inspiration: "联合预测视觉和运动可能是提升VLA预测能力的有效方法",
    pdfUrl: "https://arxiv.org/pdf/2603.10712",
    htmlUrl: "https://arxiv.org/abs/2603.10712",
    figures: []
  },
  {
    id: "self-vla-2026-03-16",
    title: "SELF-VLA: A Skill Enhanced Agentic Vision-Language-Action Framework for Contact-Rich Disassembly",
    authors: "Chang Liu, Sibo Tian, Xiao Liang et al.",
    institution: "港科大等",
    date: "2026-03-10",
    category: "vla",
    summary: "针对接触丰富拆解任务，提出技能增强的VLA智能体框架。",
    background: "拆解自动化是电子废弃物回收的关键，现有方法通常需要大量数据准备、模型训练和组件特定设计",
    architecture: "技能增强框架：结合层级化技能库与VLA",
    innovations: "任务/组件泛化能力",
    inspiration: "技能增强可能是VLA落地应用的有效路径",
    pdfUrl: "https://arxiv.org/pdf/2603.11080",
    htmlUrl: "https://arxiv.org/abs/2603.11080",
    figures: []
  },
  // === 2026-03-17 batch ===
  {
    id: "robostereo-2026-03-17",
    title: "RoboStereo: Dual-Tower 4D Embodied World Models for Unified Policy Optimization",
    authors: "Ruicheng Zhang, Guangyu Chen, Zunnan Xu et al.",
    institution: "上海人工智能实验室",
    date: "2026-03-13",
    category: "worldmodel",
    summary: "双塔4D具身世界模型，通过立体视觉建模解决几何幻觉问题，统一感知预测和策略优化。",
    background: "现有具身世界模型存在几何幻觉和缺乏统一优化框架的问题",
    architecture: "双塔架构：立体视觉塔+4D时空动态建模+统一策略优化",
    innovations: "4D（3D空间+时间）建模范式统一世界模型预测和策略优化",
    inspiration: "立体/多视角信息引入对VLA空间理解能力提升至关重要",
    pdfUrl: "https://arxiv.org/pdf/2603.12639",
    htmlUrl: "https://arxiv.org/abs/2603.12639",
    figures: []
  },
  {
    id: "rememvla-2026-03-17",
    title: "ReMem-VLA: Empowering Vision-Language-Action Model with Memory via Dual-Level Recurrent Queries",
    authors: "Hang Li, Fengyi Shen, Dong Chen et al.",
    institution: "中科院自动化所",
    date: "2026-03-13",
    category: "vla",
    summary: "双层循环查询机制为VLA赋予记忆能力，帧级查询传递细粒度信息，事件级查询压缩高层语义。",
    background: "当前VLA基于Markov假设，长时序任务中缺乏历史上下文",
    architecture: "双层可学习查询：帧级循环查询（短期）+事件级循环查询（长期）",
    innovations: "端到端记忆学习，无需外部记忆库或固定窗口",
    inspiration: "双层查询设计可借鉴到需要多步操作的灵巧手控制场景",
    pdfUrl: "https://arxiv.org/pdf/2603.12942",
    htmlUrl: "https://arxiv.org/abs/2603.12942",
    figures: []
  },
  {
    id: "tacvla-2026-03-17",
    title: "TacVLA: Contact-Aware Tactile Fusion for Robust Vision-Language-Action Manipulation",
    authors: "Kaidi Zhang, Heng Zhang, Zhengtong Xu et al.",
    institution: "多机构合作",
    date: "2026-03-13",
    category: "vla",
    summary: "将触觉模态融入VLA，接触感知动态融合策略提升遮挡和精细操作性能。",
    background: "纯视觉VLA在遮挡、精细操作和物理接触场景中性能不足",
    architecture: "触觉编码器+接触感知融合模块，根据接触状态自适应融合权重",
    innovations: "首次系统性地将触觉融入VLA微调流程",
    inspiration: "多模态感知融合是VLA走向实用的关键，触觉对精确力控制尤为重要",
    pdfUrl: "https://arxiv.org/pdf/2603.12665",
    htmlUrl: "https://arxiv.org/abs/2603.12665",
    figures: []
  },
  {
    id: "vggtworld-2026-03-17",
    title: "VGGT-World: Transforming VGGT into an Autoregressive Geometry World Model",
    authors: "Xiangyu Sun, Shijie Wang, Fengyi Zhang et al.",
    institution: "北京航空航天大学等",
    date: "2026-03-13",
    category: "worldmodel",
    summary: "用3D Gaussian Splatting替代视频帧作为世界模型预测目标，实现几何一致的场景演化预测。",
    background: "视频生成式世界模型将大量容量用于光度细节，几何不一致",
    architecture: "自回归几何世界模型，预测3D Gaussian Splatting场景演化",
    innovations: "完全绕过视频生成，用几何表示替代像素预测",
    inspiration: "几何世界模型为机器人规划直接提供3D空间信息",
    pdfUrl: "https://arxiv.org/pdf/2603.12655",
    htmlUrl: "https://arxiv.org/abs/2603.12655",
    figures: []
  },
  {
    id: "beyonddense-2026-03-17",
    title: "Beyond Dense Futures: World Models as Structured Planners for Robotic Manipulation",
    authors: "Minghao Jin, Mozheng Liao, Mingfei Han et al.",
    institution: "重庆邮电大学, 莫纳什大学",
    date: "2026-03-13",
    category: "worldmodel",
    summary: "用运动学锚定的稀疏关键帧替代密集帧预测，消除视觉冗余并减少长时序规划漂移。",
    background: "密集帧预测引入视觉冗余和误差累积，稀疏方法缺乏运动学基础",
    architecture: "结构化未来表示：稀疏关键帧运动学状态+运动学锚定",
    innovations: "在视觉前瞻和信息压缩之间取得平衡",
    inspiration: "关键帧+运动学状态设计简洁高效，适合VLA世界模型组件",
    pdfUrl: "https://arxiv.org/pdf/2603.12553",
    htmlUrl: "https://arxiv.org/abs/2603.12553",
    figures: []
  },
  {
    id: "qdig-2026-03-17",
    title: "Red-Teaming Vision-Language-Action Models via Quality Diversity Prompt Generation",
    authors: "Siddharth Srikanth, Freddie Liang, Sophie Hsu et al.",
    institution: "USC, NVIDIA",
    date: "2026-03-12",
    category: "vla",
    summary: "基于质量多样性搜索的VLA red-teaming框架，系统性发现并修复指令措辞脆弱性。",
    background: "VLA对语言指令精确措辞高度敏感，难以预测何时失败",
    architecture: "QD搜索探索多样化失败模式+对抗训练数据增强",
    innovations: "首次将QD算法系统应用于VLA red-teaming",
    inspiration: "指令鲁棒性评估必须纳入VLA产品化流程",
    pdfUrl: "https://arxiv.org/pdf/2603.12510",
    htmlUrl: "https://arxiv.org/abs/2603.12510",
    figures: []
  },
  {
    id: "atomvla-2026-03-17",
    title: "AtomVLA: Scalable Post-Training for Robotic Manipulation via Predictive Latent World Models",
    authors: "Xiaoquan Sun, Zetian Xu, Chen Cao et al.",
    institution: "多机构合作",
    date: "2026-03-09",
    category: "vla",
    summary: "通过预测性潜变量世界模型进行VLA后训练，在潜空间提供中间步骤监督。",
    background: "VLA多步行为执行中缺乏中间步骤显式引导，复合误差累积",
    architecture: "预测性潜变量世界模型+中间步骤潜空间监督+VLA后训练",
    innovations: "首次用潜变量世界模型做VLA指令定位后训练",
    inspiration: "VLA后训练是高性价比的性能提升方式，中间状态监督解决误差累积",
    pdfUrl: "https://arxiv.org/pdf/2603.08519",
    htmlUrl: "https://arxiv.org/abs/2603.08519",
    figures: []
  },
  {
    id: "cowvla-2026-03-17",
    title: "CoWVLA: Chain of World - World Model Thinking in Latent Motion (CVPR 2026)",
    authors: "Fuxiang Yang, Donglin Di, Lulu Tang et al.",
    institution: "北京理工大学, 腾讯",
    date: "2026-03-03",
    category: "vla",
    summary: "Chain-of-World范式统一世界模型时间推理和潜在运动紧凑表示，自回归联合建模稀疏关键帧和动作序列。",
    background: "世界模型VLA冗余大，潜在动作VLA缺乏连续动态建模",
    architecture: "视频VAE解耦结构/运动+世界模型预训练+联合自回归微调",
    innovations: "结构-运动解耦的潜在表示，统一两种VLA路线的优势",
    inspiration: "VLA预训练的新范式，结构-运动解耦值得借鉴",
    pdfUrl: "https://arxiv.org/pdf/2603.03195",
    htmlUrl: "https://arxiv.org/abs/2603.03195",
    figures: []
  },
  {
    id: "compact-2026-03-17",
    title: "CompACT: Planning in 8 Tokens (CVPR 2026)",
    authors: "Dongwon Kim, Gawon Seo, Jinsung Lee et al.",
    institution: "POSTECH",
    date: "2026-03-05",
    category: "worldmodel",
    summary: "将每个观测压缩到仅8个token，使世界模型规划速度提升数个数量级。",
    background: "传统tokenizer将观测编码为数百token，决策时规划计算成本过高",
    architecture: "极致压缩tokenizer+动作条件世界模型+8-token潜空间MPC",
    innovations: "8-token压缩实现世界模型实时控制部署",
    inspiration: "极端token压缩对VLA实时部署至关重要",
    pdfUrl: "https://arxiv.org/pdf/2603.05438",
    htmlUrl: "https://arxiv.org/abs/2603.05438",
    figures: []
  },
  {
    id: "spiral-2026-03-17",
    title: "SPIRAL: Self-Improving Action World Models via Reflective Planning Agents",
    authors: "Yu Yang, Yue Liao, Jianbiao Mei et al.",
    institution: "多机构合作",
    date: "2026-03-09",
    category: "worldmodel",
    summary: "闭环think-act-reflect框架让动作世界模型持续自我提升，实现可控长时序动作视频生成。",
    background: "开环ActWM一次性生成长视频导致动作不完整、时间漂移",
    architecture: "闭环反思机制：思考-行动-反思迭代+反思规划代理",
    innovations: "首次将闭环反思引入动作世界模型",
    inspiration: "闭环自改进是世界模型走向实用的关键，启发VLA在线纠错设计",
    pdfUrl: "https://arxiv.org/pdf/2603.08403",
    htmlUrl: "https://arxiv.org/abs/2603.08403",
    figures: []
  },
  {
    id: "omnistream-2026-03-17",
    title: "OmniStream: Mastering Perception, Reconstruction and Action in Continuous Streams",
    authors: "Yibin Yan, Jilan Xu, Shangzhe Di et al.",
    institution: "伦敦帝国理工学院",
    date: "2026-03-12",
    category: "ai",
    summary: "统一流式视觉骨干，同时处理感知、重建和动作，因果时空注意力实现实时推理。",
    background: "视觉基础模型各自为政，缺乏统一的流式推理能力",
    architecture: "因果时空注意力+统一骨干+物理结构化表示",
    innovations: "首个统一感知/重建/动作的流式视觉骨干",
    inspiration: "统一流式骨干是VLA部署的理想形态",
    pdfUrl: "https://arxiv.org/pdf/2603.12265",
    htmlUrl: "https://arxiv.org/abs/2603.12265",
    figures: []
  },
  {
    id: "world2act-2026-03-17",
    title: "World2Act: Latent Action Post-Training via Skill-Compositional World Models",
    authors: "An Dinh Vuong, Tuan Van Vo, Abdullah Sohail et al.",
    institution: "多机构合作",
    date: "2026-03-11",
    category: "vla",
    summary: "技能可组合的潜变量世界模型进行VLA后训练，摆脱像素空间监督限制。",
    background: "现有VLA后训练依赖像素空间监督，策略对像素级变化敏感",
    architecture: "技能可组合世界模型+潜在动作空间+VLA后训练",
    innovations: "潜空间后训练提升策略对外观变化的鲁棒性",
    inspiration: "VLA后训练作为标准流程的趋势明确",
    pdfUrl: "https://arxiv.org/pdf/2603.10422",
    htmlUrl: "https://arxiv.org/abs/2603.10422",
    figures: []
  },
  {
    id: "cosmos25-2026-03-17",
    title: "Cosmos-Predict2.5: World Simulation with Video Foundation Models for Physical AI",
    authors: "NVIDIA Team (Arslan Ali, Junjie Bai et al.)",
    institution: "NVIDIA",
    date: "2026-02-24",
    category: "worldmodel",
    summary: "NVIDIA Cosmos第二代，flow-based架构统一Text/Image/Video2World，配套Reason1 VLM和Transfer2.5 Sim2Real工具。",
    background: "物理AI需要大规模高质量合成数据，第一代Cosmos有提升空间",
    architecture: "Cosmos-Predict2.5 (2B/14B) + Cosmos-Reason1 VLM + Cosmos-Transfer2.5",
    innovations: "RL后训练提升质量，Transfer2.5小3.5x但保真度更高",
    inspiration: "Cosmos生态快速成熟，Transfer2.5的Sim2Real能力对真实机器人部署意义重大",
    pdfUrl: "https://arxiv.org/pdf/2511.00062",
    htmlUrl: "https://arxiv.org/abs/2511.00062",
    figures: []
  },
  {
    id: "playworld-2026-03-17",
    title: "PlayWorld: Learning Robot World Models from Autonomous Play",
    authors: "Tenny Yin, Zhiting Mei, Zhonghe Zheng et al.",
    institution: "多机构合作",
    date: "2026-03-09",
    category: "worldmodel",
    summary: "通过自主游戏数据训练机器人世界模型，关注物理一致性而非视觉保真度。",
    background: "当前视频模型难以预测物理一致的机器人-物体交互",
    architecture: "自主游戏数据收集+物理一致性约束的世界模型训练",
    innovations: "无任务导向的自主探索数据覆盖更广泛状态空间",
    inspiration: "自主游戏是低成本的数据收集策略，对世界模型训练有价值",
    pdfUrl: "https://arxiv.org/pdf/2603.09030",
    htmlUrl: "https://arxiv.org/abs/2603.09030",
    figures: []
  },
  {
    id: "paper-260314371",
    title: "OxyGen: Unified KV Cache Management for Vision-Language-Action Models under Multi-Task Parallelism",
    authors: "Xiangyu Li, Huaizhi Tang, Xin Ding, Weijun Wang, Ting Cao, Yunxin Liu",
    institution: "Microsoft Research Asia",
    date: "2026-03-15",
    category: "vla",
    summary: "提出统一 KV Cache 管理范式，让 π₀.₅ 等 MoT VLA 模型在多任务并行执行下实现 3.7x 加速，200+ tokens/s 语言吞吐 + 70Hz 动作频率。",
    background: "具身智能 Agent 需要并行执行多任务但 KV Cache 管理孤立导致效率低下",
    architecture: "KV Cache 作为跨任务跨时间共享资源，实现跨任务 KV 共享和跨帧连续批处理",
    innovations: "首次将 KV Cache 作为一等共享资源统一管理，连续批处理解决 LLM 推理与控制循环节奏不匹配",
    inspiration: "VLA 实际部署瓶颈从训练转向推理，资源抽象思路可推广到计算受限场景",
    pdfUrl: "https://arxiv.org/pdf/2603.14371",
    htmlUrl: "https://arxiv.org/abs/2603.14371",
    figures: []
  },
  {
    id: "paper-260313615",
    title: "EgoHOI: Egocentric World Model for Photorealistic Hand-Object Interaction Synthesis",
    authors: "Dayou Li, Lulin Liu, Bangya Liu, Shijie Zhou, Jiu Feng, Ziqi Lu, Minghui Zheng, Chenyu You, Zhiwen Fan",
    institution: "多机构合作",
    date: "2026-03-13",
    category: "worldmodel",
    summary: "首个仅从动作信号（不依赖未来物体状态）模拟真实感、物理一致的自中心手物交互世界模型。",
    background: "世界模型应从动作推断交互动力学，而非依赖已知未来状态的条件视频生成",
    architecture: "从 3D 估计蒸馏几何和运动学先验为 physics-informed embeddings 正则化生成",
    innovations: "打破已知未来状态捷径，真正 action-only 生成物理一致视频",
    inspiration: "3D 先验蒸馏作为物理教师注入世界模型，与 VLA 中 world model 组件设计高度契合",
    pdfUrl: "https://arxiv.org/pdf/2603.13615",
    htmlUrl: "https://arxiv.org/abs/2603.13615",
    figures: []
  },
  {
    id: "paper-260307648",
    title: "AtomicVLA: Unlocking the Potential of Atomic Skill Learning in Robots",
    authors: "Likui Zhang, Tao Tang, Zhihao Zhan, Xiuwei Chen, Zisheng Chen, Jianhua Han, Jiangtong Zhu, Pei Xu, Hang Xu, Hefeng Wu, Liang Lin, Xiaodan Liang",
    institution: "中山大学 / 商汤科技",
    date: "2026-03-08",
    category: "vla",
    summary: "通过 Skill-Guided MoE 实现原子技能库构建，CVPR 2026。LIBERO 超 π₀ 10%，真实世界长时序任务提升 18.3%。",
    background: "现有 VLA 单体动作解码器扩展性差，难以应对长时序多步任务和持续技能获取",
    architecture: "SG-MoE 每个专家专精一类原子技能 + 灵活路由编码器支持持续学习",
    innovations: "MoE + 技能抽象组合解决 VLA 可扩展性，动态专家组成支持终身学习",
    inspiration: "将技能作为 MoE 专家粒度比 token 级别更有语义意义，为 VLA action space 组织提供新思路",
    pdfUrl: "https://arxiv.org/pdf/2603.07648",
    htmlUrl: "https://arxiv.org/abs/2603.07648",
    figures: []
  },
  {
    id: "paper-260303596",
    title: "MEM: Multi-Scale Embodied Memory for Vision Language Action Models",
    authors: "Marcel Torne, Karl Pertsch, Homer Walke, Kyle Vedder, Suraj Nair, Brian Ichter, Allen Z. Ren, Haohuan Wang, Jiaming Tang, Kyle Stachowicz, Karan Dhabalia, Michael Equi, Quan Vuong, Jost Tobias Springenberg, Sergey Levine, Chelsea Finn, Danny Driess",
    institution: "Google DeepMind / Stanford",
    date: "2026-03-04",
    category: "vla",
    summary: "DeepMind + Stanford 联合提出视频短期记忆 + 文本长期记忆混合架构，让 VLA 能处理 15 分钟级别长时序任务。",
    background: "复杂多阶段任务需要多粒度记忆：长期语义记忆和短期感知记忆",
    architecture: "视频编码器压缩历史观测为短期记忆 + VLM 提取语义描述为长期记忆",
    innovations: "首次系统将多模态多尺度记忆引入 VLA 策略，记忆使策略能 in-context 调整操作",
    inspiration: "记忆架构是 VLA 从短操作走向长任务的关键组件，混合模态思路简单有效值得复现",
    pdfUrl: "https://arxiv.org/pdf/2603.03596",
    htmlUrl: "https://arxiv.org/abs/2603.03596",
    figures: []
  },
  {
    id: "paper-260309056",
    title: "Quality over Quantity: Demonstration Curation via Influence Functions",
    authors: "Haeone Lee, Taywon Min, Junsu Kim, Sinjae Kang, Fangchen Liu, Lerrel Pinto, Kimin Lee",
    institution: "NYU / KAIST",
    date: "2026-03-10",
    category: "embodied",
    summary: "ICRA 2026。将数据质量定义为训练样本对验证集损失贡献度，用影响函数系统筛选高质量遥操作数据。",
    background: "演示数据质量是端到端机器人控制瓶颈，数据筛选主要靠人工经验",
    architecture: "影响函数量化训练样本对验证表现的影响 + 轨迹级聚合降噪",
    innovations: "将影响函数从 NLP 引入机器人学习数据筛选，纯数据层面优化不改模型架构",
    inspiration: "VLA 训练中数据质量比数量更重要，思路可直接应用到数据管线",
    pdfUrl: "https://arxiv.org/pdf/2603.09056",
    htmlUrl: "https://arxiv.org/abs/2603.09056",
    figures: []
  },
  {
    id: "paper-260314811",
    title: "Ego to World: Collaborative Spatial Reasoning in Embodied Systems via RL",
    authors: "Heng Zhou, Li Kang, Yiran Qin, Xiufeng Song, Ao Yu, Zilu Zhang, Haoming Song, Kaixin Xu, Yuchen Fan, Dongzhan Zhou, Xiaohong Liu, Ruimao Zhang, Philip Torr, Lei Bai, Zhenfei Yin",
    institution: "多机构合作",
    date: "2026-03-16",
    category: "embodied",
    summary: "提出跨视角空间奖励机制 (CVSR)，让 VLM 在分布式多智能体场景中实现世界中心场景理解。",
    background: "多智能体具身系统中每个 Agent 只能看到受限的自中心视角",
    architecture: "CoRL: CoT SFT + GRPO RL + Cross-View Spatial Reward",
    innovations: "跨视角实体解析奖励机制，从仿真到真实多机器人操作迁移",
    inspiration: "多视角融合的 CVSR 设计可作为多模态融合参考",
    pdfUrl: "https://arxiv.org/pdf/2603.14811",
    htmlUrl: "https://arxiv.org/abs/2603.14811",
    figures: []
  },
  {
    id: "paper-260206339",
    title: "Action Hallucination in Generative Visual-Language-Action Models",
    authors: "Harold Soh, Eugene Lim",
    institution: "National University of Singapore",
    date: "2026-02-06",
    category: "vla",
    summary: "系统分析生成式 VLA 动作幻觉，提出拓扑、精度、视界三类结构性障碍及改进方向。",
    background: "VLA 替代手工规划器但是否真正解决机器人老问题存疑",
    architecture: "分析潜在变量生成式策略中的三类结构性不匹配",
    innovations: "为 VLA 部署失败提供机制性解释，指出 flow matching 等的物理约束局限",
    inspiration: "VLA 设计可能需要混合架构（生成式+约束层）来避免动作幻觉",
    pdfUrl: "https://arxiv.org/pdf/2602.06339",
    htmlUrl: "https://arxiv.org/abs/2602.06339",
    figures: []
  },
  {
    id: "paper-260212351",
    title: "LongNav-R1: Horizon-Adaptive Multi-Turn RL for Long-Horizon VLA Navigation",
    authors: "Yue Hu, Avery Xi, Qixin Xiao, Seth Isaacson, Henry X. Liu, Ram Vasudevan, Maani Ghaffari",
    institution: "University of Michigan",
    date: "2026-02-12",
    category: "vla",
    summary: "将导航重构为 VLA-环境多轮对话 + HAPO 优化，4000 rollout 让 Qwen3-VL-2B 成功率 64.3%→73.0%。",
    background: "单轮 VLA 导航无法推理历史因果效应和顺序未来结果",
    architecture: "多轮 RL + Horizon-Adaptive Policy Optimization",
    innovations: "多轮对话范式解决长时序信用分配，零样本迁移真实世界",
    inspiration: "多轮对话范式可推广到操作任务，让机器人在执行中回看并调整策略",
    pdfUrl: "https://arxiv.org/pdf/2602.12351",
    htmlUrl: "https://arxiv.org/abs/2602.12351",
    figures: []
  },
  {
    id: "paper-260219359",
    title: "Vid2Sid: Videos Can Help Close the Sim2Real Gap",
    authors: "Kevin Qiu, Yu Zhang, Marek Cygan, Josie Hughes",
    institution: "多机构合作",
    date: "2026-02-22",
    category: "embodied",
    summary: "用 VLM 分析 sim-real 视频对诊断物理参数差异，参数恢复误差<13% vs 28-98%。",
    background: "Sim2Real 仿真校准依赖人工调参或黑盒优化",
    architecture: "基础模型感知 + VLM-in-the-loop 优化器分析配对视频",
    innovations: "VLM 作为物理诊断工具，可解释的参数调整建议",
    inspiration: "VLM-in-the-loop 诊断可作为仿真校准标准工具",
    pdfUrl: "https://arxiv.org/pdf/2602.19359",
    htmlUrl: "https://arxiv.org/abs/2602.19359",
    figures: []
  },
  {
    id: "paper-260209722",
    title: "Rethinking VLA Model Scaling: Alignment, Mixture, and Regularization",
    authors: "Ye Wang, Sipeng Zheng, Hao Luo, Wanpeng Zhang, Haoqi Yuan, Chaoyi Xu, Haiweng Xu, Yicheng Feng, Mingyang Yu, Zhiyu Kang, Zongqing Lu, Qin Jin",
    institution: "多机构合作",
    date: "2026-02-10",
    category: "vla",
    summary: "系统性研究表明 VLA 暴力扩展数据常导致负迁移，EEF-relative action representation 至关重要。",
    background: "机器人数据天然异构，NLP scaling 经验不能直接套用",
    architecture: "VL backbone + flow matching 框架三维度消融：物理对齐、具身混合、正则化",
    innovations: "挑战数据越多越好假设，引入 Grouped Blind Ensemble 减少实验者偏差",
    inspiration: "VLA 训练需关注数据异构性和负迁移，不能简单套用 LLM scaling 经验",
    pdfUrl: "https://arxiv.org/pdf/2602.09722",
    htmlUrl: "https://arxiv.org/abs/2602.09722",
    figures: []
  },
  {
    id: "paper-260222818",
    title: "LeRobot: An Open-Source Library for End-to-End Robot Learning",
    authors: "Remi Cadene, Simon Aliberts, Francesco Capuano, Michel Aractingi, Adil Zouitine, Pepijn Kooijmans, Jade Choghari, Martino Russi, Caroline Pascal, Steven Palma, Mustafa Shukor, Jess Moss, Alexander Soare, Dana Aubakirova, Quentin Lhoest, Quentin Gallouédec, Thomas Wolf",
    institution: "Hugging Face",
    date: "2026-02-26",
    category: "embodied",
    summary: "Hugging Face 端到端机器人学习开源库，覆盖完整技术栈：底层通信、数据集、算法、推理。",
    background: "机器人学习被碎片化闭源工具链阻碍",
    architecture: "中间件通信 + 数据集管理 + SOTA 算法实现 + 异步推理栈",
    innovations: "首个覆盖完整机器人学习技术栈的开源库",
    inspiration: "最完整的 VLA 实验平台，标准化数据格式和算法实现方便复现对比",
    pdfUrl: "https://arxiv.org/pdf/2602.22818",
    htmlUrl: "https://arxiv.org/abs/2602.22818",
    figures: []
  },
  {
    id: "paper-260202212",
    title: "MAIN-VLA: Modeling Abstraction of Intention and eNvironment",
    authors: "Zheyuan Zhou et al.",
    institution: "多机构合作",
    date: "2026-02-02",
    category: "vla",
    summary: "意图抽象 + 环境语义抽象双通道设计，参数free token pruning，Minecraft/PvP SOTA。",
    background: "复杂动态环境中 VLA 难以从冗余传感器流提取关键动作信号",
    architecture: "Intention Abstraction + Environment Semantics Abstraction 双通道 + 注意力聚焦",
    innovations: "双抽象通道对齐产生注意力聚焦效应实现参数free token pruning",
    inspiration: "抽象作为 VLA 设计原则，高维感知压缩为结构化语义表示再与意图对齐",
    pdfUrl: "https://arxiv.org/pdf/2602.02212",
    htmlUrl: "https://arxiv.org/abs/2602.02212",
    figures: []
  },
  {
    id: "paper-260316861",
    title: "MolmoB0T: Large-Scale Simulation Enables Zero-Shot Manipulation",
    authors: "Abhay Deshpande, Dieter Fox, Ali Farhadi, Ranjay Krishna, et al.",
    date: "2026-03-17",
    institution: "NVIDIA, University of Washington",
    category: "vla",
    summary: "用超大规模仿真数据训练实现零样本sim-to-real操作迁移",
    background: "机器人学习长期共识是纯仿真不够用，MolmoB0T挑战这一假设",
    architecture: "基于Molmo VLM，大规模多样化仿真合成数据训练",
    innovations: "首次证明纯仿真数据可实现零样本真实操作",
    inspiration: "数据规模和多样性可以覆盖sim-to-real gap",
    pdfUrl: "https://arxiv.org/pdf/2603.16861",
    htmlUrl: "https://arxiv.org/abs/2603.16861",
    figures: []
  },
  {
    id: "paper-260316860",
    title: "DreamPlan: Efficient RL Fine-Tuning of VLM Planners via Video World Models",
    authors: "Emily Jia, Weiduo Yuan, Tianheng Shi, Yue Wang, et al.",
    date: "2026-03-17",
    institution: "UTS Sydney",
    category: "worldmodel",
    summary: "用视频世界模型作为虚拟环境对VLM规划器做ORPO强化微调",
    background: "直接在真实世界做RL微调代价高且不安全",
    architecture: "VLM探索数据训练视频world model，world model内ORPO微调VLM",
    innovations: "次优数据训练world model + ORPO微调替代真实交互",
    inspiration: "World model + VLM闭环，视频world model作为虚拟环境",
    pdfUrl: "https://arxiv.org/pdf/2603.16860",
    htmlUrl: "https://arxiv.org/abs/2603.16860",
    figures: []
  },
  {
    id: "paper-260316666",
    title: "Fast-WAM: Do World Action Models Need Test-time Future Imagination?",
    authors: "Tianyuan Yuan, Zibin Dong, Yicheng Liu, Hang Zhao",
    date: "2026-03-17",
    institution: "Tsinghua University",
    category: "worldmodel",
    summary: "证明WAM中视频建模价值在训练而非推理，跳过测试时预测实现4倍加速",
    background: "WAM的imagine-then-execute范式推理延迟高",
    architecture: "训练保留视频共训练，测试跳过未来预测",
    innovations: "解耦训练时world modeling和测试时future generation",
    inspiration: "World model可作为训练辅助任务而非推理组件",
    pdfUrl: "https://arxiv.org/pdf/2603.16666",
    htmlUrl: "https://arxiv.org/abs/2603.16666",
    figures: []
  },
  {
    id: "paper-260316669",
    title: "Kinema4D: Kinematic 4D World Modeling for Spatiotemporal Embodied Simulation",
    authors: "Mutian Xu, Tianbao Zhang, Xiaoguang Han, Ziwei Liu, et al.",
    date: "2026-03-17",
    institution: "CUHK-Shenzhen, NUS",
    category: "worldmodel",
    summary: "解耦运动学控制和生成式环境反应的4D机器人仿真器",
    background: "现有视频生成方案大多在2D空间操作",
    architecture: "URDF运动学生成4D轨迹 + 生成式环境反应建模",
    innovations: "运动学+生成式环境解耦，首次4D生成式仿真zero-shot迁移",
    inspiration: "4D世界模型中运动学+生成式解耦比端到端更可控",
    pdfUrl: "https://arxiv.org/pdf/2603.16669",
    htmlUrl: "https://arxiv.org/abs/2603.16669",
    figures: []
  },
  {
    id: "paper-260316195",
    title: "S-VAM: Shortcut Video-Action Model by Self-Distilling Geometric and Semantic Foresight",
    authors: "Haodong Yan, Zhide Zhong, Ying-Cong Chen, Haoang Li, et al.",
    date: "2026-03-17",
    institution: "SUSTech",
    category: "vla",
    summary: "自蒸馏将多步去噪压缩到单步推理的快捷视频动作模型",
    background: "VAMs多步生成慢、单步提取噪声大",
    architecture: "VFM多步生成教师目标，轻量化解耦器学生单步推理",
    innovations: "自蒸馏策略实现单步实时推理+高保真前视",
    inspiration: "自蒸馏加速扩散模型可推广到VLA架构",
    pdfUrl: "https://arxiv.org/pdf/2603.16195",
    htmlUrl: "https://arxiv.org/abs/2603.16195",
    figures: []
  },
  {
    id: "paper-260314522",
    title: "One-Policy-Fits-All: Geometry-Aware Action Latents for Cross-Embodiment Manipulation",
    authors: "Juncheng Mu, Huazhe Xu, Jiangmiao Pang, et al.",
    date: "2026-03-15",
    institution: "Shanghai AI Lab, Stanford",
    category: "vla",
    summary: "几何感知隐空间实现跨11种末端执行器的单策略通用操作",
    background: "跨embodiment操作面临动作空间和结构差异",
    architecture: "GaLR 3D卷积+Transformer共享隐空间 + 统一隐重定向解码器",
    innovations: "3D几何感知共享隐动作空间，8个demo适配新embodiment",
    inspiration: "3D几何信息比2D更适合动作空间对齐",
    pdfUrl: "https://arxiv.org/pdf/2603.14522",
    htmlUrl: "https://arxiv.org/abs/2603.14522",
    figures: []
  },
  {
    id: "paper-260315169",
    title: "ForceVLA2: Unleashing Hybrid Force-Position Control with Force Awareness",
    authors: "Yang Li, Cewu Lu, Yu Qiao, Jiangmiao Pang, et al.",
    date: "2026-03-16",
    institution: "SJTU, Shanghai AI Lab",
    category: "vla",
    summary: "将力感知引入VLA框架，Cross-Scale MoE实现端到端混合力位控制",
    background: "接触密集操作缺乏对交互力的显式感知",
    architecture: "VLM expert力prompt + Action expert Cross-Scale MoE融合",
    innovations: "首个端到端力感知VLA，Cross-Scale MoE力视觉融合",
    inspiration: "多模态感知融合是VLA下一重要方向",
    pdfUrl: "https://arxiv.org/pdf/2603.15169",
    htmlUrl: "https://arxiv.org/abs/2603.15169",
    figures: []
  },
  {
    id: "paper-260314523",
    title: "VLA-Thinker: Boosting VLA through Thinking-with-Image Reasoning",
    authors: "Chaoyang Wang, Yuzhang Shang, Yogesh Rawat, et al.",
    date: "2026-03-15",
    institution: "UCF",
    category: "vla",
    summary: "将视觉感知建模为可动态调用的推理动作，实现多模态embodied CoT",
    background: "现有VLA的文本CoT中视觉仅作为静态上下文",
    architecture: "SFT激活视觉CoT + GRPO对齐推理-动作轨迹",
    innovations: "视觉感知=可调用推理动作，thinking-with-image",
    inspiration: "agent需要主动回看环境的能力",
    pdfUrl: "https://arxiv.org/pdf/2603.14523",
    htmlUrl: "https://arxiv.org/abs/2603.14523",
    figures: []
  },
  {
    id: "paper-260310712",
    title: "FutureVLA: Joint Visuomotor Prediction for VLA Model",
    authors: "Xiaoxu Xu, Dahua Lin, Jiangmiao Pang, et al.",
    date: "2026-03-11",
    institution: "Shanghai AI Lab, CUHK",
    category: "vla",
    summary: "联合视动门控解耦视觉保持和动作建模，可插拔时间先验注入任意VLA",
    background: "现有方法视觉重建主导，静态场景与动态意图纠缠",
    architecture: "Joint Visuomotor Gating + latent embedding alignment",
    innovations: "视动门控解耦 + 可插拔时间先验不改变推理架构",
    inspiration: "可插拔设计降低现有VLA升级门槛",
    pdfUrl: "https://arxiv.org/pdf/2603.10712",
    htmlUrl: "https://arxiv.org/abs/2603.10712",
    figures: []
  },
  {
    id: "paper-260310448",
    title: "DiT4DiT: Jointly Modeling Video Dynamics and Actions for Robot Control",
    authors: "Teli Ma, Shuo Yang, et al.",
    date: "2026-03-11",
    institution: "Stanford",
    category: "vla",
    summary: "级联Video DiT和Action DiT，去噪中间特征作动作条件，LIBERO 98.6%",
    background: "VLA表征继承静态预训练，物理动力学学习不足",
    architecture: "Video DiT + Action DiT级联，去噪中间特征条件化",
    innovations: "去噪中间特征比重建帧更有效，样本效率10倍提升",
    inspiration: "视频生成作为robot policy scaling proxy",
    pdfUrl: "https://arxiv.org/pdf/2603.10448",
    htmlUrl: "https://arxiv.org/abs/2603.10448",
    figures: []
  },
  {
    id: "paper-260315257",
    title: "HapticVLA: Contact-Rich Manipulation without Inference-Time Tactile Sensing",
    authors: "Konstantin Gubernatorov, Dzmitry Tsetserukou, et al.",
    date: "2026-03-16",
    institution: "Skoltech",
    category: "vla",
    summary: "触觉蒸馏将触觉能力压缩进VLA，推理时无需触觉传感器",
    background: "触觉硬件增加成本，降低跨平台可复现性",
    architecture: "SA-RWFM力感知训练 + Tactile Distillation蒸馏",
    innovations: "触觉离线学习+推理时零硬件，VLA想象触觉",
    inspiration: "蒸馏思路可推广到其他难以部署的传感器模态",
    pdfUrl: "https://arxiv.org/pdf/2603.15257",
    htmlUrl: "https://arxiv.org/abs/2603.15257",
    figures: []
  },
  {
    id: "paper-260310469",
    title: "DepthCache: Depth-Guided Training-Free Visual Token Merging for VLA",
    authors: "Yuquan Li, Lianjie Ma, Han Ding, Lijun Zhu",
    date: "2026-03-11",
    institution: "HUST",
    category: "vla",
    summary: "深度引导视觉token合并，无训练1.28倍加速，性能损失不到1%",
    background: "VLA推理延迟高，现有均匀token合并破坏空间推理",
    architecture: "深度区域划分+差异化合并+时序分布+运动自适应",
    innovations: "深度结构先验指导token压缩，无训练跨架构通用",
    inspiration: "差异化压缩比均匀压缩更合理，可推广到空间结构化token",
    pdfUrl: "https://arxiv.org/pdf/2603.10469",
    htmlUrl: "https://arxiv.org/abs/2603.10469",
    figures: []
  },
];

export default papers;