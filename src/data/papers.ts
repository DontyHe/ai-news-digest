...
export const papers: Paper[] = [
  ...
  {
    id: "insight-self-guided-skill-acquisition-2026-07-06",
    title: "InSight: Self-Guided Skill Acquisition via Steerable VLAs",
    ...
    category: "embodied",
    summary: "一个通过在原始动作级别使VLA模型可转向，实现机器人自主技能获取的框架。",
    background: "当前VLA模型可以从演示中学习操作技能，但它们的能力受到训练数据中技能的限制。机器人可能面临需要新技能的情况，但这些技能不在训练数据中。传统的学习方法需要大量人类演示，而强化学习方法通常需要数千次试错。",
    architecture: "InSight系统包含两个主要阶段：1) 自动分段流水线：将演示分解为标记化的原始动作。通过VLM（视觉语言模型）计划分解和末端执行器姿态，将人类演示自动分段为原始动作。2) VLM引导的数据飞轮：识别完成新任务所需的缺失原始动作，使用VLM提出的低级控制自主尝试这些原始动作的演示，自动标记、存储并成功集成到VLA训练集中。",
    innovations: "1) 在原始动作级别实现VLA可转向性 2) 自动分段算法，无需手动标注 3) VLM引导的自主技能获取循环 4) 支持终身学习和技能组合 5) 在模拟和真实世界任务中验证",
    inspiration: "这项研究展示了如何将VLA模型从静态技能集扩展为能够持续学习的系统。对世界模型研究来说，这意味着技能的组成性原则、分层学习策略、自主探索机制、以及与世界建模的预测能力结合的重要性。",
    pdfUrl: "https://arxiv.org/pdf/2606.24884",
    htmlUrl: "https://arxiv.org/abs/2606.24884",
    figures: ["figure1.png", "figure2.png"]
  },
  {
    id: "g3vla-geometric-inductive-bias-2026-07-06",
    ...
    date: "2026-06-23",
    institution: "New York University Shanghai, Technical University of Denmark, MBZUAI, New York University Abu Dhabi",
    category: "vla",
    summary: "一种通过相机几何结构注入，提升VLA模型空间感知能力的轻量级架构。",
    background: "现有的VLA模型虽然利用了预训练的视觉-语言主干网络来支持语义理解，但其视觉接口仍然主要基于2D图像标记。这导致在多相机系统中，虽然相机内参和外参已知，但各视图被独立处理，无法充分利用几何耦合关系。这种几何-语义的不匹配限制了机器人在空间敏感任务上的表现。",
    architecture: "G³VLA框架包含三个核心几何模块：1) 内在条件射线嵌入：使用相机内参矩阵K⁻¹将每个ViT区块标记与其反投影的视线方向关联 2) 投影位置编码（PRoPE）：增强旋转位置编码，添加相机校准的跨视图投影关系偏差 3) 双向跨视图融合：在标记到达动作模型之前，交换相机流之间的几何上下文。训练过程采用两阶段：阶段1从π³X教师模型蒸馏密集点图，预训练几何模块；阶段2在动作和蒸馏损失联合作用下微调完整策略。",
    innovations: "1) 解决了VLA中2D视觉标记与校准几何结构的不匹配问题 2) 轻量级架构，不修改主干网络或动作空间 3) 支持无需深度传感器的几何监督（可从π³X获得） 4) 在多个VLA架构（π₀、π₀.₅、GR00T）上验证效果",
    inspiration: "这项研究展示了几何先验对空间任务的重要性。对世界模型研究来说，几何约束的重要性、多模态融合策略、跨视图一致性以及无监督学习都有重要启示。",
    pdfUrl: "https://arxiv.org/pdf/2606.24472",
    htmlUrl: "https://arxiv.org/abs/2606.24472",
    figures: ["overview.png", "architecture.png", "results.png"]
  },
  {
    id: "gra-geometry-guided-vla-adaptation-2026-07-06",
    title: "GRA: Geometry-Guided VLA Adaptation from Synthetic Robot Videos",
    authors: "Danze Chen, Yanzhe Chen, Qiming Huang, Zhijun Cao, Chen Gao, Mike Zheng Shou",
    date: "2026-06-23",
    institution: "National University of Singapore",
    category: "embodied",
    summary: "一种基于几何引导的VLA自适应方法，通过合成机器人视频提升训练效率。",
    background: "VLA模型需要大规模视频-动作对数据，但真实遥操作数据稀少且成本高昂。人-机器人视频生成可以大规模提供合成数据，但现有的方法通常通过恢复伪动作来训练动作头，这种方法存在根本的视频-动作不匹配问题。视频生成优化视觉合理性，而非物理控制规律，强制动作头映射视觉合成像素到真实控制信号会导致性能下降。",
    architecture: "基于不对称保存原则（Asymmetric Preservation Principle），GRA方法的核心设计是：1) 几何内容提取：从源人类视频中提取2D末端执行器航点，通过姿态估计和重定向、物理模拟、校准相机投影。2) 监督路由：几何监督通过辅助2D头路由到VLA视觉主干，动作头仅在真实演示上训练。3) 表示锚定：在微调过程中，航点目标作为空间表示锚点，防止主干向动作预测特征漂移，保持空间结构。",
    innovations: "1) 提出不对称保存原则：合成视频保存空间结构比控制动力学更可靠 2) GRA框架正确路由几何监督，避免视频-动作不匹配 3) 引入表示锚定机制，防止微调过程中几何感知表示退化 4) 在真实机器人任务上超越伪动作基线，减少对真实数据的依赖",
    inspiration: "这项研究对世界模型和数据使用效率有重要启示：数据质量比数量更重要、模块化训练策略、几何信息的关键作用以及表示持久性的重要性。",
    pdfUrl: "https://arxiv.org/pdf/2606.24448",
    htmlUrl: "https://arxiv.org/abs/2606.24448",
    figures: ["framework.png", "pipeline.png", "results.png"]
  },
  {
    id: "governing-preference-dynamics-2026-07-06",
    title: "Governing Preference Dynamics in Human–AI Interaction",
    authors: "Max Kanwal, Caryn Tran",
    date: "2026-04-01",
    institution: "Stanford University, Northwestern University",
    category: "ai",
    summary: "提出建设性对齐范式，将AI对齐视为控制人类偏好演化轨迹的问题。",
    background: "大多数AI对齐方法将人类偏好视为需要推断和优化的固定目标。这种观点与大量实证证据相矛盾，这些证据表明偏好是分层的、动态的，并通过交互（特别是与自适应技术的交互）而构建。随着AI系统变得更加持久、个性化和社会化嵌入，它们越来越多地参与塑造人们随时间关注、重视和认同的内容。",
    architecture: "建设性对齐（Constructive Alignment）范式将偏好建模为与AI系统交互下演化的分层状态变量。框架包括：1) 偏好层次结构：短期需求、工具性目标、身份认同、价值观 2) 动态变化：代际变化、生命周期变化、时间进展、上下文和状态变化、行动引起的变化 3) 控制理论框架：系统状态和动力学、未知奖励的约束优化、内在一致性、反思性认可、有限影响、认识论完整性。",
    innovations: "1) 将AI对齐从静态偏好满足问题重新定义为偏好演化轨迹的控制问题 2) 提出建设性对齐范式，关注AI系统如何影响人类偏好的形成过程 3) 使用控制理论框架来建模AI对齐 4) 强调长期价值形成而非简单满足静态偏好",
    inspiration: "虽然这篇论文主要关注AI对齐，但其对偏好动态性的理解对世界模型研究有重要启示：1) 交互学习的价值：世界模型需要在与环境的交互中不断学习和适应 2) 偏好的情境依赖性：模型需要理解任务和环境的上下文 3) 长期规划的重要性：不仅要考虑当前状态，还要考虑长期演化轨迹 4) 自我调节机制：模型需要具备反思和自我调节的能力。",
    pdfUrl: "https://arxiv.org/pdf/2607.00001",
    htmlUrl: "https://arxiv.org/abs/2607.00001",
    figures: ["framework.png", "hierarchy.png", "dynamics.png"]
  },
