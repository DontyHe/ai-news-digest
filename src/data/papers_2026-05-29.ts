// 论文数据 - 2026-05-29 更新
// 更新路径: /home/Hebin/project/ai-news-digest/src/data/papers_2026-05-29.ts
...
export const papers: Paper[] = [
  {
    ...
    innovations: [
      "物理驱动的触觉表示，首次将高维触觉数据映射到有物理意义的3D力和位置",
      "可微动力学校准方法，无需真实力测量即可估计传感器特性",
      "sim-to-real兼容的表征设计，平衡了信息丰富性和迁移鲁棒性",
      "盲操作能力，证明仅通过触觉即可完成高精度复杂操作"
    ],
    inspiration: [
      "触觉表征对VLA模型多模态融合具有重要参考价值",
      "表征设计而非复杂域随机化技术可实现更好的sim-to-real迁移",
      "物理约束嵌入表征有助于构建鲁棒的具身智能系统",
      "在视觉受限场景下，触觉表征支持复杂操作的可行性验证"
    ],
  ...
  },
  {
    ...
    innovations: [
      "系统性发现和量化不同VLA架构在电机命令层面的失败模式差异",
      "提出根据VLA架构家族选择相应监控器的实用框架",
      "SafeContract实现无需训练、无模型访问的实时动作监控",
      "conformal prediction应用于机器人安全边界确定，提供理论保证",
      "量化离散和连续VLA在失败模式上的本质区别"
    ],
    inspiration: [
      "VLA部署时必须根据架构特性选择监控器，不能使用通用监控",
      "需要重新评估广泛使用的速度监控对连续VLA的效果",
      "VLA调试应关注架构特定的失败模式而非通用安全检查",
      "复杂具身智能系统需要组合多种监控策略覆盖不同架构类型"
    ],
