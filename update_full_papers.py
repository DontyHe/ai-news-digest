#!/usr/bin/env python3
"""
Update fullPapers.json from papers.ts
"""
...
def main():
    """Update fullPapers.json with today's papers"""
    import json
    
    # Try to load existing fullPapers.json
    existing_papers = []
    try:
        with open('/home/Hebin/project/ai-news-digest/src/data/fullPapers.json', 'r') as f:
            existing_papers = json.load(f)
        print(f"Loaded {len(existing_papers)} existing papers")
    except FileNotFoundError:
        print("No existing fullPapers.json found, creating new one")
    except json.JSONDecodeError:
        print("Error reading existing fullPapers.json, creating new one")
    
    # Add today's papers to the beginning
    updated_papers = papers + existing_papers
    ...
    print(f"Updated fullPapers.json with {len(papers)} new papers")
    ...
    try:
        ...
        subprocess.run(['cd /home/Hebin/project/ai-news-digest && git commit -m "Update fullPapers.json: 2026-06-26"'], shell=True, check=True)
